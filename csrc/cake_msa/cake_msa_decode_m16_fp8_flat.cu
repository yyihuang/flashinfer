            ...
            for (int task_iter = 0; task_iter < max_task_claims + 1; task_iter++) {
                ...
                int num_n_blocks = kv_tile_end - kv_tile_begin;
                {
                    ...
                    for (int pair = 0; pair < max_decode_pairs; pair++) {
                        ...
                        for (int h = 0; h < 16; h++) {
                            {
                                __nv_bfloat16 _bval_2921307568 = __float2bfloat16_rn(exp_vals[h]);
                                uint16_t _bits_2921307568 = *(uint16_t*)&_bval_2921307568;
                                const void* _ptr_2921307568 = reinterpret_cast<const void*>((reinterpret_cast<uint8_t*>(base) + (wg_tid % 64 / 64 * 2048 + (wg_tid / 64 * 16 + h) * 128 + wg_tid % 64 % 64 * 2 ^ (wg_tid % 64 / 64 * 2048 + (wg_tid / 64 * 16 + h) * 128 + wg_tid % 64 % 64 * 2 >> 7 & 7) << 4)));
                                uint64_t _addr64_2921307568;
                                asm volatile("cvta.to.shared.u64 %0, %1;" : "=l"(_addr64_2921307568) : "l"(_ptr_2921307568));
                                uint32_t _addr_2921307568;
                                asm volatile("cvt.u32.u64 %0, %1;" : "=r"(_addr_2921307568) : "l"(_addr64_2921307568));
                                asm volatile("st.shared.b16 [%0], %1;" :: "r"(_addr_2921307568), "h"(_bits_2921307568) : "memory");
                            }
                        }
