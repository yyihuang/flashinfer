...
// Exact-shape counterfactual for the frozen B2/Q1/KV257/Hq8/Hkv1/TopK4
// boundary row.  One CTA owns one (query, head) pair.  Eight warps split the
// QK token axis, then the whole CTA performs max/sum reductions before 128
// threads accumulate one output dimension each.
__global__ __launch_bounds__(256) void flashinfer_msa_tiny_two_pass_bf16_paged(
    const __nv_bfloat16* __restrict__ q, const __nv_bfloat16* __restrict__ k,
    const __nv_bfloat16* __restrict__ v, __nv_bfloat16* __restrict__ out,
    float* __restrict__ lse, const int* __restrict__ page_table,
    const int* __restrict__ kv_lens, const int* __restrict__ q2k, int total_q,
    int num_q_heads, int num_kv_heads, int topk, int max_pages,
    float softmax_scale_log2, int is_causal) {
  constexpr int kTopK = 4;
  constexpr int kBlockSize = 128;
  constexpr int kScoreCount = kTopK * kBlockSize;
  constexpr int kWarps = 8;
  __shared__ float scores[kScoreCount];
  __shared__ float warp_partials[kWarps];
  __shared__ int physical_pages[kTopK];
  __shared__ int valid_tokens[kTopK];
  __shared__ float block_max;
  __shared__ float block_sum;

  const int work = static_cast<int>(blockIdx.x);
  const int tid = static_cast<int>(threadIdx.x);
  const int request = work / num_q_heads;
  const int q_head = work - request * num_q_heads;
  const int warp = static_cast<int>(threadIdx.x) >> 5;
  const int lane = static_cast<int>(threadIdx.x) & 31;
  if (work >= total_q * num_q_heads) {
    return;
  }

  const int group_size = num_q_heads / num_kv_heads;
  const int kv_head = q_head / group_size;
  const int q_row = work;
  const int kv_len = kv_lens[request];
  const int query_position = kv_len - 1;

  if (tid < kTopK) {
    const int selected_block = q2k[(kv_head * total_q + request) * topk + tid];
    int physical_page = -1;
    int valid = 0;
    if (selected_block >= 0 && selected_block < max_pages) {
      valid = kv_len - selected_block * kBlockSize;
      valid = valid < kBlockSize ? valid : kBlockSize;
      if (is_causal != 0) {
        const int causal_tokens = query_position - selected_block * kBlockSize + 1;
        valid = valid < causal_tokens ? valid : causal_tokens;
      }
      valid = valid > 0 ? valid : 0;
      physical_page = page_table[request * max_pages + selected_block];
      if (physical_page < 0) {
        valid = 0;
      }
    }
    physical_pages[tid] = physical_page;
    valid_tokens[tid] = valid;
  }
  scores[tid] = -CUDART_INF_F;
  scores[tid + 256] = -CUDART_INF_F;
  __syncthreads();

  float q_reg[4];
#pragma unroll
  for (int i = 0; i < 4; ++i) {
    q_reg[i] = __bfloat162float(q[q_row * 128 + lane + i * 32]);
  }

  for (int slot = 0; slot < kTopK; ++slot) {
    const int physical_page = physical_pages[slot];
    const int slot_valid = valid_tokens[slot];
    const int page_head = physical_page * num_kv_heads + kv_head;
    for (int token = warp; token < slot_valid; token += kWarps) {
      const int kv_row = (page_head * kBlockSize + token) * 128;
      float score = 0.0f;
#pragma unroll
      for (int i = 0; i < 4; ++i) {
        score += q_reg[i] * __bfloat162float(k[kv_row + lane + i * 32]);
      }
#pragma unroll
      ...
      if (lane == 0) {
        scores[slot * kBlockSize + token] = score;
      }
    }
  }
  __syncthreads();

  float local_max = fmaxf(scores[tid], scores[tid + 256]);
#pragma unroll
  for (int offset = 16; offset > 0; offset >>= 1) {
  ...
  __syncthreads();
  if (warp == 0) {
    float value = lane < kWarps ? warp_partials[lane] : -CUDART_INF_F;
#pragma unroll
    for (int offset = 16; offset > 0; offset >>= 1) {
  ...
  float weight0 = scores[tid] == -CUDART_INF_F
                      ? 0.0f
                      : exp2f((scores[tid] - block_max) * softmax_scale_log2);
  float weight1 = scores[tid + 256] == -CUDART_INF_F
                      ? 0.0f
                      : exp2f((scores[tid + 256] - block_max) * softmax_scale_log2);
  scores[tid] = weight0;
  scores[tid + 256] = weight1;
  float local_sum = weight0 + weight1;
#pragma unroll
  for (int offset = 16; offset > 0; offset >>= 1) {
  ...
  __syncthreads();
  if (warp == 0) {
    float value = lane < kWarps ? warp_partials[lane] : 0.0f;
#pragma unroll
    for (int offset = 16; offset > 0; offset >>= 1) {
  ...
  if (tid < 128) {
    ...
    for (int slot = 0; slot < kTopK; ++slot) {
      ...
      const int slot_valid = valid_tokens[slot];
      const int page_head = physical_page * num_kv_heads + kv_head;
      for (int token = 0; token < slot_valid; ++token) {
        const int kv_row = (page_head * kBlockSize + token) * 128;
        output_value +=
            scores[slot * kBlockSize + token] * __bfloat162float(v[kv_row + tid]);
      }
    }
    const float inv_sum = block_sum > 0.0f ? 1.0f / block_sum : 0.0f;
    out[q_row * 128 + tid] = __float2bfloat16_rn(output_value * inv_sum);
  }
  if (tid == 0) {
    constexpr float kLn2 = 0.6931471805599453f;
    lse[q_row] = block_sum > 0.0f
                     ? block_max * softmax_scale_log2 * kLn2 +
                           __log2f(block_sum) * kLn2
                     : -CUDART_INF_F;
  }
}
...
void Run(TensorView arg_Q, TensorView arg_Q_prefill, TensorView arg_Q_prefill_raw, TensorView arg_K,
  ...
  const bool use_tiny_boundary =
      arg_num_requests == 2 && arg_num_q_heads == 8 && arg_num_kv_heads == 1 &&
      arg_max_kv_tiles == 4 && arg_max_splits == 1 && arg_attention_mode == 1 &&
      arg_record_tasks == 1 && arg_msa_max_pages == 3 && arg_is_causal == 1 &&
      arg_Q.ndim() == 3 &&
      arg_Q.size(0) == 2 && arg_Q.size(1) == 8 && arg_Q.size(2) == 128 &&
      arg_K.ndim() == 4 && arg_K.size(0) == 6 && arg_K.size(1) == 1 &&
      arg_K.size(2) == 128 && arg_K.size(3) == 128;
  if (use_tiny_boundary) {
    void* p_q = arg_Q.data_ptr();
    void* p_k = arg_K.data_ptr();
    void* p_v = arg_V.data_ptr();
    void* p_out = arg_O.data_ptr();
    void* p_lse = arg_msa_lse.data_ptr();
    void* p_page_table = arg_kv_indices.data_ptr();
    void* p_kv_lens = arg_kv_len_arr.data_ptr();
    void* p_q2k = arg_task_kind.data_ptr();
    int32_t tiny_total_q = static_cast<int32_t>(arg_num_requests);
    int32_t tiny_num_q_heads = static_cast<int32_t>(arg_num_q_heads);
    int32_t tiny_num_kv_heads = static_cast<int32_t>(arg_num_kv_heads);
    int32_t tiny_topk = static_cast<int32_t>(arg_max_kv_tiles);
    int32_t tiny_max_pages = static_cast<int32_t>(arg_msa_max_pages);
    float tiny_softmax_scale_log2 = static_cast<float>(arg_softmax_scale_log2);
    int32_t tiny_is_causal = static_cast<int32_t>(arg_is_causal);
    void* tiny_kargs[] = {&p_q,
                          &p_k,
                          &p_v,
                          &p_out,
                          ...
                          &tiny_topk,
                          &tiny_max_pages,
                          &tiny_softmax_scale_log2,
                          &tiny_is_causal};
    cudaError_t status = cudaLaunchKernel(
        reinterpret_cast<const void*>(flashinfer_msa_tiny_two_pass_bf16_paged),
        dim3(static_cast<uint32_t>(tiny_total_q * tiny_num_q_heads), 1u, 1u),
        dim3(256u, 1u, 1u), tiny_kargs, 0u, stream);
    TVM_FFI_CHECK(status == cudaSuccess, RuntimeError)
        << "flashinfer_msa_tiny_two_pass_bf16_paged launch failed: "
