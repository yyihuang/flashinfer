...
// boundary row.  One 32-thread CTA owns one (query, head) pair and keeps the
// online-softmax output in registers; unlike the general M16 tcgen05 kernel
// this path needs neither TMEM nor dynamic shared memory.
__global__ __launch_bounds__(32) void flashinfer_msa_tiny_head_cta_bf16_paged(
...
#pragma unroll
  ...
  for (int slot = 0; slot < topk; ++slot) {
    const int selected_block = q2k[(kv_head * total_q + request) * topk + slot];
    if (selected_block < 0) {
      continue;
    }
    int valid_tokens = kv_len - selected_block * 128;
    valid_tokens = valid_tokens < 128 ? valid_tokens : 128;
    if (is_causal != 0) {
      const int causal_tokens = query_position - selected_block * 128 + 1;
      valid_tokens = valid_tokens < causal_tokens ? valid_tokens : causal_tokens;
    }
    if (valid_tokens <= 0) {
      continue;
    }
    const int physical_page = page_table[request * max_pages + selected_block];
    if (physical_page < 0) {
      continue;
    }
    const int page_head = physical_page * num_kv_heads + kv_head;
    for (int token = 0; token < valid_tokens; ++token) {
      const int kv_row = (page_head * 128 + token) * 128;
      float score = 0.0f;
#pragma unroll
      for (int i = 0; i < 4; ++i) {
...
#pragma unroll
      ...
      const float old_scale = row_max == -CUDART_INF_F
                                  ? 0.0f
                                  : exp2f((row_max - next_max) * softmax_scale_log2);
      const float value_scale = exp2f((score - next_max) * softmax_scale_log2);
      row_sum = row_sum * old_scale + value_scale;
#pragma unroll
      for (int i = 0; i < 4; ++i) {
        const float value = __bfloat162float(v[kv_row + lane + i * 32]);
        out_reg[i] = out_reg[i] * old_scale + value * value_scale;
      }
      row_max = next_max;
    }
  }
...
#pragma unroll
  for (int i = 0; i < 4; ++i) {
    out[q_row * 128 + lane + i * 32] = __float2bfloat16_rn(out_reg[i] * inv_sum);
  }
  if (lane == 0) {
    constexpr float kLn2 = 0.6931471805599453f;
    lse[q_row] = row_sum > 0.0f
                     ? row_max * softmax_scale_log2 * kLn2 + __log2f(row_sum) * kLn2
                     : -CUDART_INF_F;
  }
}
...
void Run(TensorView arg_Q, TensorView arg_Q_prefill, TensorView arg_Q_prefill_raw, TensorView arg_K,
  ...
  if (use_tiny_boundary) {
    ...
    cudaError_t status = cudaLaunchKernel(
        reinterpret_cast<const void*>(flashinfer_msa_tiny_head_cta_bf16_paged),
        dim3(static_cast<uint32_t>(tiny_total_q * tiny_num_q_heads), 1u, 1u),
        dim3(32u, 1u, 1u), tiny_kargs, 0u, stream);
    TVM_FFI_CHECK(status == cudaSuccess, RuntimeError)
        << "flashinfer_msa_tiny_head_cta_bf16_paged launch failed: "
