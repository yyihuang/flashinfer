...
// boundary row.  One warp owns one query head and keeps the online-softmax
// output in registers; unlike the general M16 tcgen05 kernel this path needs
// neither TMEM nor dynamic shared memory.
__global__ __launch_bounds__(256) void flashinfer_msa_tiny_bf16_paged(
  ...
  if (request >= total_q || warp >= num_q_heads) {
    return;
  }

  const int group_size = num_q_heads / num_kv_heads;
  const int kv_head = warp / group_size;
  const int q_row = request * num_q_heads + warp;
  float q_reg[4];
...
void Run(TensorView arg_Q, TensorView arg_Q_prefill, TensorView arg_Q_prefill_raw, TensorView arg_K,
  ...
  const bool use_tiny_boundary =
      ...
      arg_record_tasks == 1 && arg_msa_max_pages == 3 && arg_Q.ndim() == 3 &&
  ...
  if (use_tiny_boundary) {
    ...
    cudaError_t status = cudaLaunchKernel(
        reinterpret_cast<const void*>(flashinfer_msa_tiny_bf16_paged),
        dim3(static_cast<uint32_t>(tiny_total_q), 1u, 1u), dim3(256u, 1u, 1u), tiny_kargs, 0u,
        stream);
    TVM_FFI_CHECK(status == cudaSuccess, RuntimeError)
        << "flashinfer_msa_tiny_bf16_paged launch failed: " << cudaGetErrorString(status);
    return;
  }
