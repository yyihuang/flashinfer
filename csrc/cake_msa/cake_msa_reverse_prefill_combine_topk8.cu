...
#define SMEM_WEIGHTS_STAGE_BYTES 2048
#define SMEM_WEIGHTS_STRIDE 2048
...
kernel_cake_msa_reverse_prefill_combine_topk8(__nv_bfloat16* __restrict__ partial_o, float* __restrict__ partial_lse, float* __restrict__ partial_temperature_lse, int* __restrict__ split_counts, __nv_bfloat16* __restrict__ out, float* __restrict__ lse, float* __restrict__ temperature_lse, int total_q, int num_q_heads, int num_kv_heads, int qhead_per_kv, int topk, int return_softmax_lse, int return_temperature_lse)
{
    ...
    if (tid < 64) {
        ...
        if (row < total_rows_out) {
            ...
            float lse_sum = 0.0f;
            #pragma unroll 1
            for (int split_1 = 0; split_1 < 8; split_1++) {
                float weight = 0.0f;
                ...
                weights[local_row * 8 + split_1] = weight;
                lse_sum += weight;
            }
            float _rcp_0 = approx_rcp(lse_sum);
            float inv_lse_sum = ((lse_sum > 0.0f && lse_sum == lse_sum) ? _rcp_0 : 0.0f);
            #pragma unroll 1
            for (int split_2 = 0; split_2 < 8; split_2++) {
                weights[local_row * 8 + split_2] = weights[local_row * 8 + split_2] * inv_lse_sum;
            }
            ...
            if (return_temperature_lse != 0) {
                ...
                #pragma unroll 1
                for (int split_3 = 0; split_3 < 8; split_3++) {
                ...
                #pragma unroll 1
                for (int split_4 = 0; split_4 < 8; split_4++) {
    ...
    for (int wave = 0; wave < 4; wave++) {
        ...
        if (row_1 < total_rows_out) {
            ...
            #pragma unroll 1
            for (int split_5 = 0; split_5 < 8; split_5++) {
                if (split_count_1 > split_5) {
                    ...
                    float weight_1 = weights[local_row_1 * 8 + split_5];
                    #pragma unroll
