...
kernel_cake_msa_decode_m16_fp8_paged(const __grid_constant__ CakeMsaTensorMap Q_value, const __grid_constant__ CakeMsaTensorMap Q_prefill_value, __nv_bfloat16* __restrict__ Q_prefill_raw, const __grid_constant__ CakeMsaTensorMap K_value, const __grid_constant__ CakeMsaTensorMap K_prefill_pair_value, const __grid_constant__ CakeMsaTensorMap V_value, const __grid_constant__ CakeMsaTensorMap V_prefill_pair_value, const __grid_constant__ CakeMsaTensorMap KV_value, __nv_bfloat16* __restrict__ O, float* __restrict__ partial_O, float* __restrict__ partial_M, float* __restrict__ partial_D, int* __restrict__ split_completion, float* __restrict__ msa_lse, int* __restrict__ kv_indices, int* __restrict__ qo_indptr, int* __restrict__ kv_indptr, int* __restrict__ kv_len_arr, int* __restrict__ task_kind, int* __restrict__ task_request, int* __restrict__ task_kv_head, int* __restrict__ task_q_tile, int* __restrict__ task_split, int* __restrict__ task_kv_tile_begin, int* __restrict__ task_kv_tile_end, int* __restrict__ task_qo_begin, int* __restrict__ task_qo_end, int* __restrict__ task_page_begin, int* __restrict__ task_page_end, int* __restrict__ status, int num_requests, int num_q_heads, int num_kv_heads, int max_kv_tiles, int max_splits, int max_task_claims, float softmax_scale_log2, int attention_mode, int is_causal, int derive_q_offset, int record_tasks, int msa_max_pages, int msa_split_policy)
{
            ...
            for (int task_iter = 0; task_iter < max_task_claims + 1; task_iter++) {
                ...
                int num_n_blocks = kv_tile_end - kv_tile_begin;
                {
                    ...
                    for (int pair = 0; pair < max_decode_pairs; pair++) {
                        ...
                        for (int h = 0; h < 16; h++) {
                            {
                                __nv_bfloat16 _bval_1862573008 = __float2bfloat16_rn(exp_vals[h]);
                                uint16_t _bits_1862573008 = *(uint16_t*)&_bval_1862573008;
                                const void* _ptr_1862573008 = reinterpret_cast<const void*>((reinterpret_cast<uint8_t*>(base) + (wg_tid % 64 / 64 * 2048 + (wg_tid / 64 * 16 + h) * 128 + wg_tid % 64 % 64 * 2 ^ (wg_tid % 64 / 64 * 2048 + (wg_tid / 64 * 16 + h) * 128 + wg_tid % 64 % 64 * 2 >> 7 & 7) << 4)));
                                uint64_t _addr64_1862573008;
                                asm volatile("cvta.to.shared.u64 %0, %1;" : "=l"(_addr64_1862573008) : "l"(_ptr_1862573008));
                                uint32_t _addr_1862573008;
                                asm volatile("cvt.u32.u64 %0, %1;" : "=r"(_addr_1862573008) : "l"(_addr64_1862573008));
                                asm volatile("st.shared.b16 [%0], %1;" :: "r"(_addr_1862573008), "h"(_bits_1862573008) : "memory");
                            }
                        }
        ...
        { // producer_aux_main
            ...
            unsigned int _phase_work_full_1_0_4 = 0;
            {
                ...
                for (int task_iter_4 = 0; task_iter_4 < max_task_claims + 1; task_iter_4++) {
                    ...
                    #pragma unroll 1
                    for (int ni_3 = 0; ni_3 < max_kv_tiles; ni_3++) {
