...
kernel_cake_msa_reverse_prefill_fp8_flat(const __grid_constant__ CakeMsaTensorMap k_value, const __grid_constant__ CakeMsaTensorMap v_value, __nv_bfloat16* __restrict__ q, int* __restrict__ scheduler_metadata, int* __restrict__ k2q_row_ptr, int* __restrict__ k2q_qsplit_indices, __nv_bfloat16* __restrict__ partial_o, float* __restrict__ partial_lse, float* __restrict__ partial_temperature_lse, int* __restrict__ cu_seqlens_q, int* __restrict__ cu_seqlens_k, int* __restrict__ q_offsets, int* __restrict__ kv_lens, int* __restrict__ page_table, int total_q, int num_q_heads, int num_kv_heads, int total_rows, int nnz_per_head, int work_capacity, int num_work_items, int topk, int max_pages, int causal, int derive_q_offset, float softmax_scale_log2, float lse_temperature_scale, int return_temperature_lse)
{
    ...
    if (warp <= 3) {
        ...
        { // softmax_main
            ...
            for (int group = 0; group < 21; group++) {
                ...
                int token_in_group = my_row / 16;
                int edge_in_work = group * 8 + token_in_group;
    ...
    // ---- Role: store ----
        ...
        { // store_main
            ...
            int token_in_group_1 = packed_row / 16;
            int q_head_local = packed_row - token_in_group_1 * 16;
            int tmem_row_base_1 = (warp - 4) * 32 << 16;
            unsigned int o_full_phase = 0;
            #pragma unroll 1
            for (int group_1 = 0; group_1 < 21; group_1++) {
                ...
                int edge_in_work_1 = group_1 * 8 + token_in_group_1;
    ...
    // ---- Role: qload ----
        ...
        { // qload_main
            ...
            #pragma unroll 1
            for (int group_2 = 0; group_2 < 21; group_2++) {
                mbarrier_wait(q_empty_addr, q_empty_phase);
                q_empty_phase = q_empty_phase ^ 1;
                asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
                int owned_packed_2 = -1;
                int owned_edge = group_2 * 8 + lane;
                if (lane < 8 && owned_edge < q_count_2) {
                ...
                for (int vector_iteration = 0; vector_iteration < 16; vector_iteration++) {
                    ...
                    int token_in_group_2 = packed_row_1 / 16;
                    int q_head_local_1 = packed_row_1 - token_in_group_2 * 16;
                    ...
                    if (edge_in_work_2 < q_count_2) {
                        ...
                        int q_head_1 = head_kv_2 * 16 + q_head_local_1;
        ...
        { // transform_main
            ...
            if (warp == 13) {
                if (elect_sync()) {
                    ...
                    mbarrier_arrive(fp8_empty_addr);
                }
            }
            unsigned int _phase_fp8_v_full_0 = 0;
            mbarrier_wait(fp8_v_full_addr, _phase_fp8_v_full_0);
            _phase_fp8_v_full_0 ^= 1;
            asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
            {
                const char* _src_ptr = smem_raw + (fp8_smem_addr - smem);
                char* _dst_ptr = smem_raw + (v_convert_smem_addr - smem);
                const int _tid = (int)threadIdx.x - (13) * 32;
                #pragma unroll 4
                for (int _off = _tid; _off < 2048; _off += 64) {
                    uint64_t _src64 = reinterpret_cast<const uint64_t*>(_src_ptr)[_off];
    ...
    // ---- Role: load_warp ----
        ...
        { // load_warp_main
            ...
            mbarrier_wait(fp8_empty_addr, 0);
            if (elect_sync()) {
                mbarrier_arrive_expect_tx(fp8_v_full_addr, 16384);
                int token0_1 = token_base;
                int token1_1 = token_base + 64;
                tma_3d_gmem2smem(fp8_smem_addr, v, 0, token0_1, page_head, fp8_v_full_addr);
                tma_3d_gmem2smem(fp8_smem_addr + 8192, v, 0, token1_1, page_head, fp8_v_full_addr);
            }
        }
    }
