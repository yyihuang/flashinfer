...
// tvm-ffi direct-source launcher for Cake kernel 'kernel_cake_kimi_k3_mla_fp8_paged_attention_366073d9fac05fa435b9'.
#include <cuda.h>
#include <cuda_bf16.h>
#include <cuda_fp16.h>
...
extern "C" __global__ void kernel_cake_kimi_k3_mla_fp8_paged_attention_366073d9fac05fa435b9(const __grid_constant__ CUtensorMap tmap_q, const __grid_constant__ CUtensorMap tmap_k, const __grid_constant__ CUtensorMap tmap_v, __nv_bfloat16* __restrict__ partial_O, float* __restrict__ partial_max, float* __restrict__ partial_sum, int* __restrict__ seq_lens, int* __restrict__ cum_seq_lens_q, int* __restrict__ page_table, float softmax_scale_log2, float bmm2_scale, int num_heads, int num_split, int max_pages_per_seq);


namespace cake_host_shim_b1aa3950afcd8732 {
...
void Run(TensorView arg_tmap_q, TensorView arg_tmap_k, TensorView arg_tmap_v, TensorView arg_partial_O, TensorView arg_partial_max, TensorView arg_partial_sum, TensorView arg_seq_lens, TensorView arg_cum_seq_lens_q, TensorView arg_page_table, double arg_softmax_scale_log2, double arg_bmm2_scale, int64_t arg_num_heads, int64_t arg_num_split, int64_t arg_max_pages_per_seq, int64_t grid_x, int64_t grid_y, int64_t grid_z) {
  ...
  void* kargs[] = {&p_tmap_q, &p_tmap_k, &p_tmap_v, &p_partial_O, &p_partial_max, &p_partial_sum, &p_seq_lens, &p_cum_seq_lens_q, &p_page_table, &v_softmax_scale_log2, &v_bmm2_scale, &v_num_heads, &v_num_split, &v_max_pages_per_seq};
    ...
    if (it == smem_status_by_device->end()) {
      ...
      if (smem_status == cudaSuccess) {
        smem_status = cudaFuncSetAttribute(
            reinterpret_cast<const void*>(kernel_cake_kimi_k3_mla_fp8_paged_attention_366073d9fac05fa435b9),
  ...
  TVM_FFI_CHECK(smem_status == cudaSuccess, RuntimeError)
      << "cudaFuncSetAttribute for kernel_cake_kimi_k3_mla_fp8_paged_attention_366073d9fac05fa435b9 failed: "
  ...
  cudaError_t launch_status = cudaLaunchKernelExC(
      &config, reinterpret_cast<const void*>(kernel_cake_kimi_k3_mla_fp8_paged_attention_366073d9fac05fa435b9), kargs);
  TVM_FFI_CHECK(launch_status == cudaSuccess, RuntimeError)
      << "cudaLaunchKernelExC for kernel_cake_kimi_k3_mla_fp8_paged_attention_366073d9fac05fa435b9 failed: "
      << cudaGetErrorString(launch_status);
}

}  // namespace cake_host_shim_b1aa3950afcd8732

TVM_FFI_DLL_EXPORT_TYPED_FUNC(run, cake_host_shim_b1aa3950afcd8732::Run);
