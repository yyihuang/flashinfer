...
kernel_cake_kimi_k3_mla_fp8_paged_attention_366073d9fac05fa435b9(const __grid_constant__ CUtensorMap tmap_q, const __grid_constant__ CUtensorMap tmap_k, const __grid_constant__ CUtensorMap tmap_v, __nv_bfloat16* __restrict__ partial_O, float* __restrict__ partial_max, float* __restrict__ partial_sum, int* __restrict__ seq_lens, int* __restrict__ cum_seq_lens_q, int* __restrict__ page_table, float softmax_scale_log2, float bmm2_scale, int num_heads, int num_split, int max_pages_per_seq)
{
                        ...
                        if (tile > 0) {
                            if (any_rescale != 0) {
                                ...
                                asm volatile("tcgen05.wait::ld.sync.aligned;");
                                {
                                    #pragma unroll
                                    for (int j = 0; j < 32; j++) {
                                        _tmem_load_0[j] = _tmem_load_0[j] * acc_scale;
                                        _tmem_load_1[j] = _tmem_load_1[j] * acc_scale;
                                    }
                                }
                                ...
                                asm volatile("tcgen05.wait::ld.sync.aligned;");
                                {
                                    ...
                                    for (int j_1 = 0; j_1 < 32; j_1++) {
                                        _tmem_load_2[j_1] = _tmem_load_2[j_1] * acc_scale;
                                        _tmem_load_3[j_1] = _tmem_load_3[j_1] * acc_scale;
                                    }
                                }
                ...
                for (int c2 = 0; c2 < 128; c2 += 128) {
                        ...
                        : "r"(o_base_epi + c2 + 64 + 32));
                    {
                        ...
                        for (int _ls = 0; _ls < 16; _ls++)
                            mul_f32x2_inplace(&reinterpret_cast<float2*>((_tmem_load_4 + 0))[_ls], _scale2_1);
                        #pragma unroll
                        ...
                        #pragma unroll
                        for (int _ls = 0; _ls < 16; _ls++)
                            mul_f32x2_inplace(&reinterpret_cast<float2*>((_tmem_load_4 + 32))[_ls], _scale2_2);
                        #pragma unroll
                        for (int _lp = 0; _lp < 16; _lp++) {
                        ...
                        for (int j_3 = 0; j_3 < 16; j_3 += 4) {
                            asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" :: "r"(smem_epi_addr + (unsigned int)(epi_wbase + (epi_wkey ^ j_3 / 4) * 16)), "r"(packed_epi[j_3]), "r"(packed_epi[j_3 + 1]), "r"(packed_epi[j_3 + 2]), "r"(packed_epi[j_3 + 3]) : "memory");
                        }
                        __syncwarp();
                        #pragma unroll
                    ...
                    }
                    {
                        ...
                        #pragma unroll
                        for (int _ls = 0; _ls < 16; _ls++)
                            mul_f32x2_inplace(&reinterpret_cast<float2*>((_tmem_load_5 + 0))[_ls], _scale2_3);
                        #pragma unroll
                        for (int _lp = 0; _lp < 16; _lp++) {
                        ...
                        #pragma unroll
                        for (int _ls = 0; _ls < 16; _ls++)
                            mul_f32x2_inplace(&reinterpret_cast<float2*>((_tmem_load_5 + 32))[_ls], _scale2_4);
                        #pragma unroll
                        for (int _lp = 0; _lp < 16; _lp++) {
                        ...
                        for (int j_5 = 0; j_5 < 16; j_5 += 4) {
                            asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" :: "r"(smem_epi_addr + (unsigned int)(epi_wbase + (epi_wkey ^ j_5 / 4) * 16)), "r"(packed_epi_1[j_5]), "r"(packed_epi_1[j_5 + 1]), "r"(packed_epi_1[j_5 + 2]), "r"(packed_epi_1[j_5 + 3]) : "memory");
                        }
                        __syncwarp();
                        #pragma unroll
                        ...
                        if (any_rescale_1 != 0) {
                            ...
                            asm volatile("tcgen05.wait::ld.sync.aligned;");
                            {
                                ...
                                for (int j_6 = 0; j_6 < 32; j_6++) {
                                    _tmem_load_6[j_6] = _tmem_load_6[j_6] * acc_scale_1;
                                    _tmem_load_7[j_6] = _tmem_load_7[j_6] * acc_scale_1;
                                }
                            }
                            ...
                            asm volatile("tcgen05.wait::ld.sync.aligned;");
                            {
                                ...
                                for (int j_7 = 0; j_7 < 32; j_7++) {
                                    _tmem_load_8[j_7] = _tmem_load_8[j_7] * acc_scale_1;
                                    _tmem_load_9[j_7] = _tmem_load_9[j_7] * acc_scale_1;
                                }
                            }
            ...
            for (int vs_local_1 = 0; vs_local_1 < 1; vs_local_1++) {
                ...
                for (int c2_1 = 0; c2_1 < 128; c2_1 += 128) {
                        ...
                        : "r"(o_base_epi_1 + c2_1 + 64 + 32));
                    {
                        ...
                        const float2 _scale2_19 = {output_scale_1, output_scale_1};
                        #pragma unroll
                        for (int _ls = 0; _ls < 16; _ls++)
                            mul_f32x2_inplace(&reinterpret_cast<float2*>((_tmem_load_10 + 0))[_ls], _scale2_19);
                        #pragma unroll
                        for (int _lp = 0; _lp < 16; _lp++) {
                        ...
                        for (int j_8 = 0; j_8 < 16; j_8 += 4) {
                            asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" :: "r"(smem_epi_addr + (unsigned int)(epi_wbase_1 + (epi_wkey_1 ^ j_8 / 4) * 16)), "r"(packed_epi_2[j_8]), "r"(packed_epi_2[j_8 + 1]), "r"(packed_epi_2[j_8 + 2]), "r"(packed_epi_2[j_8 + 3]) : "memory");
                        }
                        __syncwarp();
                        #pragma unroll
                        ...
                        #pragma unroll
                        for (int _ls = 0; _ls < 16; _ls++)
                            mul_f32x2_inplace(&reinterpret_cast<float2*>((_tmem_load_10 + 32))[_ls], _scale2_20);
                        #pragma unroll
                        for (int _lp = 0; _lp < 16; _lp++) {
                        ...
                        for (int j_9 = 0; j_9 < 16; j_9 += 4) {
                            asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" :: "r"(smem_epi_addr + (unsigned int)(epi_wbase_1 + (epi_wkey_1 ^ j_9 / 4) * 16)), "r"(packed_epi_2[j_9]), "r"(packed_epi_2[j_9 + 1]), "r"(packed_epi_2[j_9 + 2]), "r"(packed_epi_2[j_9 + 3]) : "memory");
                        }
                        __syncwarp();
                        #pragma unroll
                    ...
                    }
                    {
                        ...
                        const float2 _scale2_21 = {output_scale_1, output_scale_1};
                        #pragma unroll
                        for (int _ls = 0; _ls < 16; _ls++)
                            mul_f32x2_inplace(&reinterpret_cast<float2*>((_tmem_load_11 + 0))[_ls], _scale2_21);
                        #pragma unroll
                        for (int _lp = 0; _lp < 16; _lp++) {
                        ...
                        for (int j_10 = 0; j_10 < 16; j_10 += 4) {
                            asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" :: "r"(smem_epi_addr + (unsigned int)(epi_wbase_1 + (epi_wkey_1 ^ j_10 / 4) * 16)), "r"(packed_epi_3[j_10]), "r"(packed_epi_3[j_10 + 1]), "r"(packed_epi_3[j_10 + 2]), "r"(packed_epi_3[j_10 + 3]) : "memory");
                        }
                        __syncwarp();
                        #pragma unroll
                        ...
                        #pragma unroll
                        for (int _ls = 0; _ls < 16; _ls++)
                            mul_f32x2_inplace(&reinterpret_cast<float2*>((_tmem_load_11 + 32))[_ls], _scale2_22);
                        #pragma unroll
                        for (int _lp = 0; _lp < 16; _lp++) {
                        ...
                        for (int j_11 = 0; j_11 < 16; j_11 += 4) {
                            asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" :: "r"(smem_epi_addr + (unsigned int)(epi_wbase_1 + (epi_wkey_1 ^ j_11 / 4) * 16)), "r"(packed_epi_3[j_11]), "r"(packed_epi_3[j_11 + 1]), "r"(packed_epi_3[j_11 + 2]), "r"(packed_epi_3[j_11 + 3]) : "memory");
                        }
                        __syncwarp();
                        #pragma unroll
