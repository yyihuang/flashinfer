...
// tvm-ffi direct-source launcher for Cake kernel 'kernel_cake_kimi_k3_mla_fp8_paged_attention_a68072b28bedc16b32b6'.
#include <cuda.h>
#include <cuda_bf16.h>
#include <cuda_fp16.h>
...
void Run(TensorView arg_tmap_q, TensorView arg_tmap_k, TensorView arg_tmap_qr, TensorView arg_tmap_kr, TensorView arg_tmap_v, TensorView arg_partial_O, TensorView arg_partial_max, TensorView arg_partial_sum, TensorView arg_seq_lens, TensorView arg_cum_seq_lens_q, TensorView arg_page_table, double arg_softmax_scale_log2, double arg_bmm2_scale, int64_t arg_num_heads, int64_t arg_num_split, int64_t arg_max_pages_per_seq, int64_t grid_x, int64_t grid_y, int64_t grid_z) {
    ...
    if (it == smem_status_by_device->end()) {
      ...
      if (smem_status == cudaSuccess) {
        smem_status = cudaFuncSetAttribute(
            reinterpret_cast<const void*>(kernel_cake_kimi_k3_mla_fp8_paged_attention_a68072b28bedc16b32b6),
  ...
  TVM_FFI_CHECK(smem_status == cudaSuccess, RuntimeError)
      << "cudaFuncSetAttribute for kernel_cake_kimi_k3_mla_fp8_paged_attention_a68072b28bedc16b32b6 failed: "
  ...
  cudaError_t launch_status = cudaLaunchKernelExC(
      &config, reinterpret_cast<const void*>(kernel_cake_kimi_k3_mla_fp8_paged_attention_a68072b28bedc16b32b6), kargs);
  TVM_FFI_CHECK(launch_status == cudaSuccess, RuntimeError)
      << "cudaLaunchKernelExC for kernel_cake_kimi_k3_mla_fp8_paged_attention_a68072b28bedc16b32b6 failed: "
      << cudaGetErrorString(launch_status);
}

}  // namespace cake_host_shim_cf573155a185267b

TVM_FFI_DLL_EXPORT_TYPED_FUNC(run, cake_host_shim_cf573155a185267b::Run);
