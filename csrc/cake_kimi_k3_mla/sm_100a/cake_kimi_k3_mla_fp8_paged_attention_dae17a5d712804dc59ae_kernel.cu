...
kernel_cake_kimi_k3_mla_fp8_paged_attention_dae17a5d712804dc59ae(const __grid_constant__ CUtensorMap tmap_q, const __grid_constant__ CUtensorMap tmap_k, const __grid_constant__ CUtensorMap tmap_v, __nv_bfloat16* __restrict__ partial_O, float* __restrict__ partial_max, float* __restrict__ partial_sum, int* __restrict__ seq_lens, int* __restrict__ cum_seq_lens_q, int* __restrict__ page_table, float softmax_scale_log2, float bmm2_scale, int num_heads, int num_split, int max_pages_per_seq)
{
    ...
    #define k_empty_addr (mbar_base + 112)
    ...
    #define sum_ready_addr (mbar_base + 320)
    #define o_full_addr (mbar_base + 328)
    #define o_done_addr (mbar_base + 344)
    #define partial_results_ready_addr (mbar_base + 352)
    #define q_pair_ready_addr (mbar_base + 360)
    #define tmem_dealloc_peer_addr (mbar_base + 368)
    #define tmem_epoch_ready_addr (mbar_base + 376)
    ...
    uint8_t* smem_v = reinterpret_cast<uint8_t*>(smem_raw + 140288);
    const int smem_v_addr = smem + 140288;
    float* smem_stats_max = reinterpret_cast<float*>(smem_raw + 205824);
    const int smem_stats_max_addr = smem + 205824;
    float* smem_stats_sum = reinterpret_cast<float*>(smem_raw + 207872);
    const int smem_stats_sum_addr = smem + 207872;
    uint8_t* smem_p = reinterpret_cast<uint8_t*>(smem_raw + 209920);
    const int smem_p_addr = smem + 209920;
    ...
    if (warp == 0) {
        ...
        if (leader) {
            ...
            // --- pipeline 'k_pipe' ---
            // k_full: 12 barriers, init_count=1
            ...
            // k_empty: 12 barriers, init_count=1
            ...
            mbarrier_init(smem + 264, 16);
            // p_full: 2 barriers, init_count=16
            mbarrier_init(smem + 272, 16);
            mbarrier_init(smem + 280, 16);
                ...
                int phase = tile & 1;
                {
                    ...
                    float local_max = -CAKE_INF;
                    {
                        ...
                        if (sm_tail < 32) {
                            ...
                            if (!(_slice_lo_mask_1 & (1u << 0))) sv[0] = -CAKE_INF;
                            if (!(_slice_lo_mask_1 & (1u << 1))) sv[1] = -CAKE_INF;
                            if (!(_slice_lo_mask_1 & (1u << 2))) sv[2] = -CAKE_INF;
                            if (!(_slice_lo_mask_1 & (1u << 3))) sv[3] = -CAKE_INF;
                            if (!(_slice_lo_mask_1 & (1u << 4))) sv[4] = -CAKE_INF;
                            if (!(_slice_lo_mask_1 & (1u << 5))) sv[5] = -CAKE_INF;
                            if (!(_slice_lo_mask_1 & (1u << 6))) sv[6] = -CAKE_INF;
                            if (!(_slice_lo_mask_1 & (1u << 7))) sv[7] = -CAKE_INF;
                            if (!(_slice_lo_mask_1 & (1u << 8))) sv[8] = -CAKE_INF;
                            if (!(_slice_lo_mask_1 & (1u << 9))) sv[9] = -CAKE_INF;
                            if (!(_slice_lo_mask_1 & (1u << 10))) sv[10] = -CAKE_INF;
                            if (!(_slice_lo_mask_1 & (1u << 11))) sv[11] = -CAKE_INF;
                            if (!(_slice_lo_mask_1 & (1u << 12))) sv[12] = -CAKE_INF;
                            if (!(_slice_lo_mask_1 & (1u << 13))) sv[13] = -CAKE_INF;
                            if (!(_slice_lo_mask_1 & (1u << 14))) sv[14] = -CAKE_INF;
                            if (!(_slice_lo_mask_1 & (1u << 15))) sv[15] = -CAKE_INF;
                            if (!(_slice_lo_mask_1 & (1u << 16))) sv[16] = -CAKE_INF;
                            if (!(_slice_lo_mask_1 & (1u << 17))) sv[17] = -CAKE_INF;
                            if (!(_slice_lo_mask_1 & (1u << 18))) sv[18] = -CAKE_INF;
                            if (!(_slice_lo_mask_1 & (1u << 19))) sv[19] = -CAKE_INF;
                            if (!(_slice_lo_mask_1 & (1u << 20))) sv[20] = -CAKE_INF;
                            if (!(_slice_lo_mask_1 & (1u << 21))) sv[21] = -CAKE_INF;
                            if (!(_slice_lo_mask_1 & (1u << 22))) sv[22] = -CAKE_INF;
                            if (!(_slice_lo_mask_1 & (1u << 23))) sv[23] = -CAKE_INF;
                            if (!(_slice_lo_mask_1 & (1u << 24))) sv[24] = -CAKE_INF;
                            if (!(_slice_lo_mask_1 & (1u << 25))) sv[25] = -CAKE_INF;
                            if (!(_slice_lo_mask_1 & (1u << 26))) sv[26] = -CAKE_INF;
                            if (!(_slice_lo_mask_1 & (1u << 27))) sv[27] = -CAKE_INF;
                            if (!(_slice_lo_mask_1 & (1u << 28))) sv[28] = -CAKE_INF;
                            if (!(_slice_lo_mask_1 & (1u << 29))) sv[29] = -CAKE_INF;
                            if (!(_slice_lo_mask_1 & (1u << 30))) sv[30] = -CAKE_INF;
                            if (!(_slice_lo_mask_1 & (1u << 31))) sv[31] = -CAKE_INF;
                        }
                        float _max3_0;
                    ...
                    new_max = ((lazy_delta < -2.0f) ? new_max : row_max_val);
                    ...
                    int any_rescale = 0;
                    {
                        int _vote_1 = __any_sync(0xFFFFFFFF, acc_scale < 1.0f);
                        any_rescale = _vote_1;
                    }
                    row_max_val = new_max;
                    ...
                    float max_scaled = safe_max * softmax_scale_log2 - 6.8073549220576f;
                    {
                        ...
                        if (tile > 0) {
                            if (any_rescale != 0) {
                                int o_ready_stage = tile - 1 & 1;
                                int o_ready_phase = tile - 1 >> 1 & 1;
                                mbarrier_wait(o_full_addr + (o_ready_stage) * 8, o_ready_phase);
                                asm volatile("tcgen05.fence::after_thread_sync;");
                                int o_rs_base = taddr + 256 + (unsigned int)(warp % 4 * 32 << 16);
                                ...
                                asm volatile("tcgen05.wait::ld.sync.aligned;");
                                {
                                    const float2 _scale2_20 = {acc_scale, acc_scale};
                                    #pragma unroll
                                    for (int _ls = 0; _ls < 16; _ls++)
                                        mul_f32x2_inplace(&reinterpret_cast<float2*>(_tmem_load_0)[_ls], _scale2_20);
                                    const float2 _scale2_21 = {acc_scale, acc_scale};
                                    #pragma unroll
                                    for (int _ls = 0; _ls < 16; _ls++)
                                        mul_f32x2_inplace(&reinterpret_cast<float2*>(_tmem_load_1)[_ls], _scale2_21);
                                }
                                tmem_st_x32_f32(o_rs_base, _tmem_load_0);
                                tmem_st_x32_f32(o_rs_base + 32, _tmem_load_1);
                                ...
                                asm volatile("tcgen05.wait::ld.sync.aligned;");
                                {
                                    const float2 _scale2_22 = {acc_scale, acc_scale};
                                    #pragma unroll
                                    for (int _ls = 0; _ls < 16; _ls++)
                                        mul_f32x2_inplace(&reinterpret_cast<float2*>(_tmem_load_2)[_ls], _scale2_22);
                                    const float2 _scale2_23 = {acc_scale, acc_scale};
                                    #pragma unroll
                                    for (int _ls = 0; _ls < 16; _ls++)
                                        mul_f32x2_inplace(&reinterpret_cast<float2*>(_tmem_load_3)[_ls], _scale2_23);
                                }
                                tmem_st_x32_f32(o_rs_base + 64, _tmem_load_2);
                                tmem_st_x32_f32(o_rs_base + 96, _tmem_load_3);
            ...
            for (int vs_local = 1; vs_local < 2; vs_local++) {
                ...
                for (int c2 = 0; c2 < 128; c2 += 128) {
                        ...
                        : "r"(o_base_epi + c2 + 64 + 32));
                    {
                        ...
                        const float2 _scale2_24 = {output_scale, output_scale};
                        #pragma unroll
                        for (int _ls = 0; _ls < 16; _ls++)
                            mul_f32x2_inplace(&reinterpret_cast<float2*>((_tmem_load_4 + 0))[_ls], _scale2_24);
                        #pragma unroll
                    ...
                    }
                    {
                        ...
                        #pragma unroll
                        for (int _ls = 0; _ls < 16; _ls++)
                            mul_f32x2_inplace(&reinterpret_cast<float2*>((_tmem_load_5 + 32))[_ls], _scale2_27);
                        #pragma unroll
                        for (int _lp = 0; _lp < 16; _lp++) {
        ...
        { // correction_wg_main
            ...
            for (int tile_1 = 0; tile_1 < my_n_tiles_1; tile_1++) {
                int phase_1 = tile_1 & 1;
                {
                    ...
                    if (local_tail_1 < 32) {
                        ...
                        if (!(_slice_lo_mask_3 & (1u << 0))) sv_1[0] = -CAKE_INF;
                        if (!(_slice_lo_mask_3 & (1u << 1))) sv_1[1] = -CAKE_INF;
                        if (!(_slice_lo_mask_3 & (1u << 2))) sv_1[2] = -CAKE_INF;
                        if (!(_slice_lo_mask_3 & (1u << 3))) sv_1[3] = -CAKE_INF;
                        if (!(_slice_lo_mask_3 & (1u << 4))) sv_1[4] = -CAKE_INF;
                        if (!(_slice_lo_mask_3 & (1u << 5))) sv_1[5] = -CAKE_INF;
                        if (!(_slice_lo_mask_3 & (1u << 6))) sv_1[6] = -CAKE_INF;
                        if (!(_slice_lo_mask_3 & (1u << 7))) sv_1[7] = -CAKE_INF;
                        if (!(_slice_lo_mask_3 & (1u << 8))) sv_1[8] = -CAKE_INF;
                        if (!(_slice_lo_mask_3 & (1u << 9))) sv_1[9] = -CAKE_INF;
                        if (!(_slice_lo_mask_3 & (1u << 10))) sv_1[10] = -CAKE_INF;
                        if (!(_slice_lo_mask_3 & (1u << 11))) sv_1[11] = -CAKE_INF;
                        if (!(_slice_lo_mask_3 & (1u << 12))) sv_1[12] = -CAKE_INF;
                        if (!(_slice_lo_mask_3 & (1u << 13))) sv_1[13] = -CAKE_INF;
                        if (!(_slice_lo_mask_3 & (1u << 14))) sv_1[14] = -CAKE_INF;
                        if (!(_slice_lo_mask_3 & (1u << 15))) sv_1[15] = -CAKE_INF;
                        if (!(_slice_lo_mask_3 & (1u << 16))) sv_1[16] = -CAKE_INF;
                        if (!(_slice_lo_mask_3 & (1u << 17))) sv_1[17] = -CAKE_INF;
                        if (!(_slice_lo_mask_3 & (1u << 18))) sv_1[18] = -CAKE_INF;
                        if (!(_slice_lo_mask_3 & (1u << 19))) sv_1[19] = -CAKE_INF;
                        if (!(_slice_lo_mask_3 & (1u << 20))) sv_1[20] = -CAKE_INF;
                        if (!(_slice_lo_mask_3 & (1u << 21))) sv_1[21] = -CAKE_INF;
                        if (!(_slice_lo_mask_3 & (1u << 22))) sv_1[22] = -CAKE_INF;
                        if (!(_slice_lo_mask_3 & (1u << 23))) sv_1[23] = -CAKE_INF;
                        if (!(_slice_lo_mask_3 & (1u << 24))) sv_1[24] = -CAKE_INF;
                        if (!(_slice_lo_mask_3 & (1u << 25))) sv_1[25] = -CAKE_INF;
                        if (!(_slice_lo_mask_3 & (1u << 26))) sv_1[26] = -CAKE_INF;
                        if (!(_slice_lo_mask_3 & (1u << 27))) sv_1[27] = -CAKE_INF;
                        if (!(_slice_lo_mask_3 & (1u << 28))) sv_1[28] = -CAKE_INF;
                        if (!(_slice_lo_mask_3 & (1u << 29))) sv_1[29] = -CAKE_INF;
                        if (!(_slice_lo_mask_3 & (1u << 30))) sv_1[30] = -CAKE_INF;
                        if (!(_slice_lo_mask_3 & (1u << 31))) sv_1[31] = -CAKE_INF;
                    }
                    float _max3_15;
                    #if defined(__CUDA_ARCH__) && (__CUDA_ARCH__ < 1000)
                    #error "Max3 requires PTX three-input max.f32 support on sm_100+"
                    ...
                    sv_1[31] = _fma_pair_18.y;
                    {
                        ...
                        for (int _le = 0; _le < 16; _le++) {
                            if (1 && _le >= 12) {
                                float2 _exp2_pair_19 = ex2_emulation_f32x2_value(make_float2(sv_1[_le*2], sv_1[_le*2 + 1]));
                                sv_1[_le*2] = _exp2_pair_19.x;
                                sv_1[_le*2 + 1] = _exp2_pair_19.y;
                            } else {
                    ...
                    if (tile_1 > 0) {
                        if (any_rescale_1 != 0) {
                            int o_ready_stage_1 = tile_1 - 1 & 1;
                            int o_ready_phase_1 = tile_1 - 1 >> 1 & 1;
                            mbarrier_wait(o_full_addr + (o_ready_stage_1) * 8, o_ready_phase_1);
                            asm volatile("tcgen05.fence::after_thread_sync;");
                            int o_rs_base_1 = taddr + 256 + 128 + (unsigned int)(warp % 4 * 32 << 16);
                            ...
                            asm volatile("tcgen05.wait::ld.sync.aligned;");
                            {
                                const float2 _scale2_20 = {acc_scale_1, acc_scale_1};
                                #pragma unroll
                                for (int _ls = 0; _ls < 16; _ls++)
                                    mul_f32x2_inplace(&reinterpret_cast<float2*>(_tmem_load_6)[_ls], _scale2_20);
                                const float2 _scale2_21 = {acc_scale_1, acc_scale_1};
                                #pragma unroll
                                for (int _ls = 0; _ls < 16; _ls++)
                                    mul_f32x2_inplace(&reinterpret_cast<float2*>(_tmem_load_7)[_ls], _scale2_21);
                            }
                            tmem_st_x32_f32(o_rs_base_1, _tmem_load_6);
                            tmem_st_x32_f32(o_rs_base_1 + 32, _tmem_load_7);
                            ...
                            asm volatile("tcgen05.wait::ld.sync.aligned;");
                            {
                                const float2 _scale2_22 = {acc_scale_1, acc_scale_1};
                                #pragma unroll
                                for (int _ls = 0; _ls < 16; _ls++)
                                    mul_f32x2_inplace(&reinterpret_cast<float2*>(_tmem_load_8)[_ls], _scale2_22);
                                const float2 _scale2_23 = {acc_scale_1, acc_scale_1};
                                #pragma unroll
                                for (int _ls = 0; _ls < 16; _ls++)
                                    mul_f32x2_inplace(&reinterpret_cast<float2*>(_tmem_load_9)[_ls], _scale2_23);
                            }
                            tmem_st_x32_f32(o_rs_base_1 + 64, _tmem_load_8);
                            tmem_st_x32_f32(o_rs_base_1 + 96, _tmem_load_9);
                            asm volatile("tcgen05.wait::st.sync.aligned;" ::: "memory");
                            asm volatile("tcgen05.fence::before_thread_sync;");
                        }
                    }
            ...
            if (n_half_1 == 0) {
                if (r_local_1 < rows_valid_1) {
                    ...
                    float stored_max = ((final_row_max == -CAKE_INF) ? -CAKE_INF : row_max_scaled);
            ...
            for (int vs_local_1 = 0; vs_local_1 < 1; vs_local_1++) {
                ...
                for (int c2_1 = 0; c2_1 < 128; c2_1 += 128) {
                        ...
                        : "r"(o_base_epi_1 + c2_1 + 64 + 32));
                    {
                        ...
                        #pragma unroll
                        for (int _ls = 0; _ls < 16; _ls++)
                            mul_f32x2_inplace(&reinterpret_cast<float2*>((_tmem_load_10 + 0))[_ls], _scale2_24);
                        #pragma unroll
                        for (int _lp = 0; _lp < 16; _lp++) {
                        ...
                        #pragma unroll
                        for (int _ls = 0; _ls < 16; _ls++)
                            mul_f32x2_inplace(&reinterpret_cast<float2*>((_tmem_load_10 + 32))[_ls], _scale2_25);
                        #pragma unroll
                        for (int _lp = 0; _lp < 16; _lp++) {
                    ...
                    }
                    {
                        ...
                        #pragma unroll
                        for (int _ls = 0; _ls < 16; _ls++)
                            mul_f32x2_inplace(&reinterpret_cast<float2*>((_tmem_load_11 + 0))[_ls], _scale2_26);
                        #pragma unroll
                        for (int _lp = 0; _lp < 16; _lp++) {
                        ...
                        #pragma unroll
                        for (int _ls = 0; _ls < 16; _ls++)
                            mul_f32x2_inplace(&reinterpret_cast<float2*>((_tmem_load_11 + 32))[_ls], _scale2_27);
                        #pragma unroll
                        for (int _lp = 0; _lp < 16; _lp++) {
                ...
                if (my_n_tiles_2 > tile_2) {
                    ...
                    if (cta_rank == 0) {
                        ...
                        asm volatile("tcgen05.fence::after_thread_sync;");
                        {
                            ...
                            int _mma_b_lo_0 = (((smem_k_addr) >> 4) & 0x3FFF) + (mma_k_stage) * 512;
                            asm volatile(
                    "{\n\t"
                    ...
                    mma_k_stage += 1;
                    if (mma_k_stage == 12) { mma_k_stage = 0; _phase_k_full ^= 1; }
                    if (cta_rank == 0) {
                        mbarrier_wait(k_full_addr + (mma_k_stage) * 8, _phase_k_full);
                        asm volatile("tcgen05.fence::after_thread_sync;");
                        {
                            int _mma_a_lo_2 = (((smem_q_addr + 8192) >> 4) & 0x3FFF) + (0) * 2560;
                            int _mma_b_lo_2 = (((smem_k_addr) >> 4) & 0x3FFF) + (mma_k_stage) * 512;
                            asm volatile(
                    "{\n\t"
                    ".reg .pred leader, p0, p1;\n\t"
                    ".reg .b32 adhi, bdhi, alo, blo, id, m0, m1, m2, m3, m4, m5, m6, m7;\n\t"
                    ...
                    if (cta_rank == 0) {
                        ...
                        asm volatile("tcgen05.fence::after_thread_sync;");
                        {
                            ...
                            int _mma_b_lo_4 = (((smem_k_addr) >> 4) & 0x3FFF) + (mma_k_stage) * 512;
                            asm volatile(
                    "{\n\t"
                    ".reg .pred leader, p0, p1;\n\t"
                    ".reg .b32 adhi, bdhi, alo, blo, id, m0, m1, m2, m3, m4, m5, m6, m7;\n\t"
                    ...
                    if (cta_rank == 0) {
                        ...
                        asm volatile("tcgen05.fence::after_thread_sync;");
                        {
                            ...
                            int _mma_b_lo_6 = (((smem_k_addr) >> 4) & 0x3FFF) + (mma_k_stage) * 512;
                            asm volatile(
                    "{\n\t"
                    ".reg .pred leader, p0, p1;\n\t"
                    ".reg .b32 adhi, bdhi, alo, blo, id, m0, m1, m2, m3, m4, m5, m6, m7;\n\t"
                    ...
                    if (cta_rank == 0) {
                        ...
                        asm volatile("tcgen05.fence::after_thread_sync;");
                        {
                            ...
                            int _mma_b_lo_9 = (((smem_k_addr) >> 4) & 0x3FFF) + (mma_k_stage) * 512;
                            asm volatile(
                    "{\n\t"
                    ".reg .pred leader, p0, p1;\n\t"
                    ".reg .b32 adhi, bdhi, alo, blo, id, m0, m1, m2, m3, m4, m5, m6, m7;\n\t"
    ...
    if (warp == 11) {
        { // load_warp_main
            ...
            int q_row_global = row_base_global_3 + cta_rank * 64;
            ...
            int slot_ok = ((n_pages_3 > 2 * my_start_block_3 + cta_rank) ? 2 * my_start_block_3 + cta_rank : 0);
            int pg_k = page_table[pt_base_3 + slot_ok];
                    ...
                    if (cta_rank == 0) {
                        if (elect_sync()) {
                            asm volatile(
                                ...
                                :: "r"((k_full_addr + (load_k_stage) * 8) & 0xFEFFFFFF), "r"((uint32_t)(16384)) : "memory");
                        }
                    }
                    if (elect_sync()) {
                        tma_2d_gmem2smem_cta2(smem_k_addr + load_k_stage * 8192, (&tmap_k), n * 128, pg_k * 64, ((k_full_addr + (load_k_stage) * 8) & 0xFEFFFFFF));
                    }
                }
                load_k_stage += 1;
                if (load_k_stage == 12) { load_k_stage = 0; _phase_k_empty ^= 1; }
            }
            if (my_n_tiles_3 > 1) {
                int slot_ok_0 = ((n_pages_3 > 2 * (my_start_block_3 + 1) + cta_rank) ? 2 * (my_start_block_3 + 1) + cta_rank : 0);
                int pg_k_1 = page_table[pt_base_3 + slot_ok_0];
                #pragma unroll
                for (int n_1 = 0; n_1 < 5; n_1++) {
                    mbarrier_wait(k_empty_addr + (load_k_stage) * 8, _phase_k_empty);
                    {
                        ...
                        if (elect_sync()) {
                            tma_2d_gmem2smem_cta2(smem_k_addr + load_k_stage * 8192, (&tmap_k), n_1 * 128, pg_k_1 * 64, ((k_full_addr + (load_k_stage) * 8) & 0xFEFFFFFF));
                        }
                    }
                    load_k_stage += 1;
                    if (load_k_stage == 12) { load_k_stage = 0; _phase_k_empty ^= 1; }
            ...
            for (int tile_3 = 0; tile_3 < my_n_tiles_3; tile_3++) {
                if (tile_3 > 0) {
                    ...
                    int slot_ok_0_1 = ((n_pages_3 > 2 * abs_tile_v) ? 2 * abs_tile_v : 0);
                    int pg_v0 = page_table[pt_base_3 + slot_ok_0_1];
                    int slot_ok_1 = ((n_pages_3 > 2 * abs_tile_v + 1) ? 2 * abs_tile_v + 1 : 0);
                    int pg_v1 = page_table[pt_base_3 + slot_ok_1];
                ...
                if (my_n_tiles_3 > tile_3 + 2) {
                    int slot_ok_0_2 = ((n_pages_3 > 2 * (my_start_block_3 + tile_3 + 2) + cta_rank) ? 2 * (my_start_block_3 + tile_3 + 2) + cta_rank : 0);
                    int pg_k_1_1 = page_table[pt_base_3 + slot_ok_0_2];
                    #pragma unroll
                    for (int n_2 = 0; n_2 < 5; n_2++) {
                        mbarrier_wait(k_empty_addr + (load_k_stage) * 8, _phase_k_empty);
                        {
                            ...
                            if (elect_sync()) {
                                tma_2d_gmem2smem_cta2(smem_k_addr + load_k_stage * 8192, (&tmap_k), n_2 * 128, pg_k_1_1 * 64, ((k_full_addr + (load_k_stage) * 8) & 0xFEFFFFFF));
                            }
                        }
                        load_k_stage += 1;
                        if (load_k_stage == 12) { load_k_stage = 0; _phase_k_empty ^= 1; }
            ...
            int slot_ok_0_3 = ((n_pages_3 > 2 * abs_tile_last) ? 2 * abs_tile_last : 0);
            int pg_vl0 = page_table[pt_base_3 + slot_ok_0_3];
            int slot_ok_1_1 = ((n_pages_3 > 2 * abs_tile_last + 1) ? 2 * abs_tile_last + 1 : 0);
            int pg_vl1 = page_table[pt_base_3 + slot_ok_1_1];
            mbarrier_wait(v_empty_addr + (load_v_stage) * 8, _phase_v_empty);
            {
                if (cta_rank == 0) {
                    if (elect_sync()) {
