...
kernel_gdn_decode_pretranspose_mtp_t4_bf16state_wide128(__nv_bfloat16* __restrict__ q, __nv_bfloat16* __restrict__ k, __nv_bfloat16* __restrict__ v, __nv_bfloat16* __restrict__ state, float* __restrict__ A_log, __nv_bfloat16* __restrict__ a, float* __restrict__ dt_bias, __nv_bfloat16* __restrict__ b, __nv_bfloat16* __restrict__ out, __nv_bfloat16* __restrict__ intermediate_state, int* __restrict__ initial_state_indices, int* __restrict__ output_state_indices, long long state_stride_p0, long long q_stride_p0, long long q_stride_p1, long long q_stride_p2, long long k_stride_p0, long long k_stride_p1, long long k_stride_p2, long long a_stride_p0, long long a_stride_p1, long long a_stride_p2, long long b_stride_p0, long long b_stride_p1, long long b_stride_p2, long long v_stride_p0, long long v_stride_p1, long long v_stride_p2, long long state_pool_size)
{
    ...
    bool read_state_valid = read_state_slot_raw >= 0 && (long long)read_state_slot_raw < state_pool_size;
    int read_state_slot = ((read_state_valid) ? read_state_slot_raw : 0);
    int write_state_slot_raw = output_state_indices[n];
    bool write_state_valid = read_state_valid && write_state_slot_raw >= 0 && (long long)write_state_slot_raw < state_pool_size;
    int write_state_slot = ((write_state_valid) ? write_state_slot_raw : 0);
    ...
    bool read_state_aligned = read_state_head_base % 8 == 0;
    bool write_state_aligned = write_state_head_base % 8 == 0;
    ...
    for (int iter_idx = 0; iter_idx < iters_per_group; iter_idx++) {
        ...
        long long state_row_base = read_state_head_base + (long long)(v_row_a * 128) + (long long)k_start;
        if (read_state_aligned) {
            {
                const uint4* _vptr_2 = reinterpret_cast<const uint4*>(state + state_row_base);
                uint4 _vld_2[1];
                #pragma unroll
                for (int _blk = 0; _blk < 1; _blk++) {
            ...
            #pragma unroll
            for (int state_elem = 0; state_elem < 8; state_elem++) {
                r_h[state_elem] = (float)state[state_row_base + (long long)state_elem];
            }
        }
        ...
        state_row_base = read_state_head_base + (long long)(v_row_b * 128) + (long long)k_start;
        if (read_state_aligned) {
            {
                const uint4* _vptr_3 = reinterpret_cast<const uint4*>(state + state_row_base);
                uint4 _vld_3[1];
                #pragma unroll
                for (int _blk = 0; _blk < 1; _blk++) {
            ...
            #pragma unroll
            for (int state_elem_1 = 0; state_elem_1 < 8; state_elem_1++) {
                r_h[state_elem_1] = (float)state[state_row_base + (long long)state_elem_1];
            }
        }
        ...
        state_row_base = read_state_head_base + (long long)(v_row_c * 128) + (long long)k_start;
        if (read_state_aligned) {
            {
                const uint4* _vptr_4 = reinterpret_cast<const uint4*>(state + state_row_base);
                uint4 _vld_4[1];
                #pragma unroll
                for (int _blk = 0; _blk < 1; _blk++) {
            ...
            #pragma unroll
            for (int state_elem_2 = 0; state_elem_2 < 8; state_elem_2++) {
                r_h[state_elem_2] = (float)state[state_row_base + (long long)state_elem_2];
            }
        }
        ...
        state_row_base = read_state_head_base + (long long)(v_row_d * 128) + (long long)k_start;
        if (read_state_aligned) {
            {
                const uint4* _vptr_5 = reinterpret_cast<const uint4*>(state + state_row_base);
                uint4 _vld_5[1];
                #pragma unroll
                for (int _blk = 0; _blk < 1; _blk++) {
            ...
            #pragma unroll
            for (int state_elem_3 = 0; state_elem_3 < 8; state_elem_3++) {
                r_h[state_elem_3] = (float)state[state_row_base + (long long)state_elem_3];
            }
        }
        ...
        if (UPDATE_STATE && write_state_valid) {
            r_h[0] = h_a_pair0.x;
            r_h[1] = h_a_pair0.y;
            r_h[2] = h_a_pair1.x;
            r_h[3] = h_a_pair1.y;
            ...
            r_h[6] = h_a_pair3.x;
            r_h[7] = h_a_pair3.y;
            state_row_base = write_state_head_base + (long long)(v_row_a * 128) + (long long)k_start;
            if (write_state_aligned) {
                {
                ...
                #pragma unroll
                for (int state_elem_4 = 0; state_elem_4 < 8; state_elem_4++) {
                    __nv_bfloat16 _cvt_bf16_0 = __float2bfloat16(r_h[state_elem_4]);
                    state[state_row_base + (long long)state_elem_4] = _cvt_bf16_0;
                }
            }
            r_h[0] = h_b_pair0.x;
            r_h[1] = h_b_pair0.y;
            ...
            r_h[6] = h_b_pair3.x;
            r_h[7] = h_b_pair3.y;
            state_row_base = write_state_head_base + (long long)(v_row_b * 128) + (long long)k_start;
            if (write_state_aligned) {
                {
                    __nv_bfloat162 _pk[4];
                    _pk[0] = __floats2bfloat162_rn(r_h[0 + 0], r_h[0 + 1]);
                ...
                #pragma unroll
                for (int state_elem_5 = 0; state_elem_5 < 8; state_elem_5++) {
                    __nv_bfloat16 _cvt_bf16_1 = __float2bfloat16(r_h[state_elem_5]);
                    state[state_row_base + (long long)state_elem_5] = _cvt_bf16_1;
                }
            }
            r_h[0] = h_c_pair0.x;
            r_h[1] = h_c_pair0.y;
            ...
            r_h[6] = h_c_pair3.x;
            r_h[7] = h_c_pair3.y;
            state_row_base = write_state_head_base + (long long)(v_row_c * 128) + (long long)k_start;
            if (write_state_aligned) {
                {
                    __nv_bfloat162 _pk[4];
                    _pk[0] = __floats2bfloat162_rn(r_h[0 + 0], r_h[0 + 1]);
                ...
                #pragma unroll
                for (int state_elem_6 = 0; state_elem_6 < 8; state_elem_6++) {
                    __nv_bfloat16 _cvt_bf16_2 = __float2bfloat16(r_h[state_elem_6]);
                    state[state_row_base + (long long)state_elem_6] = _cvt_bf16_2;
                }
            }
            r_h[0] = h_d_pair0.x;
            r_h[1] = h_d_pair0.y;
            ...
            r_h[6] = h_d_pair3.x;
            r_h[7] = h_d_pair3.y;
            state_row_base = write_state_head_base + (long long)(v_row_d * 128) + (long long)k_start;
            if (write_state_aligned) {
                {
                    __nv_bfloat162 _pk[4];
                    _pk[0] = __floats2bfloat162_rn(r_h[0 + 0], r_h[0 + 1]);
                ...
                #pragma unroll
                for (int state_elem_7 = 0; state_elem_7 < 8; state_elem_7++) {
                    __nv_bfloat16 _cvt_bf16_3 = __float2bfloat16(r_h[state_elem_7]);
                    state[state_row_base + (long long)state_elem_7] = _cvt_bf16_3;
                }
            }
        }
    }
