...
kernel_gdn_decode_pretranspose_splitv8(__nv_bfloat16* __restrict__ q, __nv_bfloat16* __restrict__ k, __nv_bfloat16* __restrict__ v, float* __restrict__ state, float* __restrict__ A_log, __nv_bfloat16* __restrict__ a, float* __restrict__ dt_bias, __nv_bfloat16* __restrict__ b, __nv_bfloat16* __restrict__ out, int* __restrict__ initial_state_indices, int* __restrict__ output_state_indices, long long state_pool_size)
{
    ...
    int safe_read_state_slot = ((read_state_valid) ? read_state_slot_raw : 0);
    ...
    int safe_write_state_slot = ((write_state_valid) ? write_state_slot_raw : 0);
    ...
    for (int copy_iter = 0; copy_iter < 4; copy_iter++) {
        ...
        asm volatile("cp.async.cg.shared::cta.global [%0], [%1], 16, %2;"
            :: "r"(copy_dst), "l"(state + (read_state_head_base + (long long)(copy_v_row * 128) + (long long)copy_k_base)), "r"((read_state_valid) ? 16 : 0));
    }
    ...
    for (int tile = 0; tile < 1; tile++) {
        ...
        if (tile + 1 < 1) {
            ...
            for (int copy_iter_1 = 0; copy_iter_1 < 4; copy_iter_1++) {
                ...
                asm volatile("cp.async.cg.shared::cta.global [%0], [%1], 16, %2;"
                    :: "r"(copy_dst_1), "l"(state + (read_state_head_base + (long long)(copy_v_row_1 * 128) + (long long)copy_k_base_1)), "r"((read_state_valid) ? 16 : 0));
            }
            asm volatile("cp.async.commit_group;");
        }
        #pragma unroll
        for (int row_group = 0; row_group < 16; row_group += 4) {
            ...
            if (lane_local == 0) {
                out[vh_base + v_row] = ((read_state_valid) ? sum_hq : 0.0f);
            }
        }
    }
}
