            ...
            for (unsigned int tile = bid; tile < total_tiles; tile += num_bids) {
                ...
                for (int chunk_idx = 0; chunk_idx < num_chunks_b; chunk_idx += 2) {
                    ...
                    unsigned int ainv1_stage = ((1) ? (ainv0_stage + 1) % 3 : ainv_cg0_stage);
                    {
                        ...
                        for (int qk_repeat_cg0_1 = 0; qk_repeat_cg0_1 < 4; qk_repeat_cg0_1++) {
                            ...
                            for (int _ls = 0; _ls < 4; _ls++)
                                reinterpret_cast<float2*>(_t2)[_ls] = mul_f32x2(reinterpret_cast<float2*>((_tmem_load_2 + qk_j0_cg0))[_ls], reinterpret_cast<const float2*>((qk_transfer0_cg0 + qk_j0_cg0))[_ls]);
                            const float2 _scale2_0 = {kk_beta0_cg0, kk_beta0_cg0};
                            #pragma unroll
                            for (int _ls = 0; _ls < 1; _ls++)
                                mul_f32x2_inplace(&reinterpret_cast<float2*>((_t2 + 0))[_ls], _scale2_0);
                            const float2 _scale2_1 = {kk_beta1_cg0, kk_beta1_cg0};
                            #pragma unroll
                            for (int _ls = 0; _ls < 1; _ls++)
                                mul_f32x2_inplace(&reinterpret_cast<float2*>((_t2 + 2))[_ls], _scale2_1);
                            const float2 _scale2_2 = {kk_beta0_cg0, kk_beta0_cg0};
                            #pragma unroll
                            for (int _ls = 0; _ls < 1; _ls++)
                                mul_f32x2_inplace(&reinterpret_cast<float2*>((_t2 + 4))[_ls], _scale2_2);
                            const float2 _scale2_3 = {kk_beta1_cg0, kk_beta1_cg0};
                            #pragma unroll
                            for (int _ls = 0; _ls < 1; _ls++)
                                mul_f32x2_inplace(&reinterpret_cast<float2*>((_t2 + 6))[_ls], _scale2_3);
                            uint32_t _t2_bf16[4];
                            #pragma unroll
                            ...
                            asm volatile("stmatrix.sync.aligned.m8n8.x4.shared.b16 [%0], {%1, %2, %3, %4};\n"
                                :: "r"(_stmatrix_addr_4), "r"(*reinterpret_cast<const uint32_t*>(&_t2_bf16[0])), "r"(*reinterpret_cast<const uint32_t*>(&_t2_bf16[1])), "r"(*reinterpret_cast<const uint32_t*>(&_t2_bf16[2])), "r"(*reinterpret_cast<const uint32_t*>(&_t2_bf16[3]))
                                : "memory");
                        }
                    ...
                    for (int qk_repeat_cg0_2 = 0; qk_repeat_cg0_2 < 4; qk_repeat_cg0_2++) {
                        ...
                        #pragma unroll
                        for (int _ls = 0; _ls < 4; _ls++)
                            reinterpret_cast<float2*>(_t3)[_ls] = mul_f32x2(reinterpret_cast<float2*>((_tmem_load_3 + qk_j0_cg0_1))[_ls], reinterpret_cast<const float2*>((qk_transfer0_cg0 + qk_j0_cg0_1))[_ls]);
                        const float2 _scale2_14 = {scale, scale};
                        #pragma unroll
                        for (int _ls = 0; _ls < 4; _ls++)
                            mul_f32x2_inplace(&reinterpret_cast<float2*>(_t3)[_ls], _scale2_14);
                        uint32_t _t3_bf16[4];
                        #pragma unroll
                        for (int _lp = 0; _lp < 4; _lp++) {
                        ...
                        asm volatile("stmatrix.sync.aligned.m8n8.x4.shared.b16 [%0], {%1, %2, %3, %4};\n"
                            :: "r"(_stmatrix_addr_15), "r"(*reinterpret_cast<const uint32_t*>(&_t3_bf16[0])), "r"(*reinterpret_cast<const uint32_t*>(&_t3_bf16[1])), "r"(*reinterpret_cast<const uint32_t*>(&_t3_bf16[2])), "r"(*reinterpret_cast<const uint32_t*>(&_t3_bf16[3]))
                            : "memory");
                    }
                    asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
                    mbarrier_arrive(cg0_shared_acc_empty_addr);
