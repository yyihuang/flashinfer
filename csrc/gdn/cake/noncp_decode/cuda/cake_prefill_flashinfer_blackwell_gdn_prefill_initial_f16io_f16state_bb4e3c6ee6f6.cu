            ...
            for (unsigned int tile_1 = bid; tile_1 < total_tiles; tile_1 += num_bids) {
                        ...
                        for (int state_vec_init = 0; state_vec_init < 8; state_vec_init++) {
                            {
                                ...
                                for (int _pair = 0; _pair < 2; _pair++) {
                                    asm volatile(
                                        ...
                                        "mov.b64 %0, {f_lo, f_hi};\n\t"
                                        "}\n"
                                        : "=l"(*reinterpret_cast<unsigned long long*>(&state_init_frag[state_vec_init * 4 + _pair * 2]))
                                        : "r"(_vpairs_0[_pair]));
                                }
                            }
                        }
                ...
                if ((STORE_FINAL_STATE != 0 || ENABLE_CHECKPOINTS != 0) && num_chunks_b_1 > 0) {
                    ...
                    for (int state_col_block_cg1 = 0; state_col_block_cg1 < 4; state_col_block_cg1++) {
                        ...
                        #pragma unroll
                        for (int state_vec_idx_cg1 = 0; state_vec_idx_cg1 < ((1) ? 4 : 8); state_vec_idx_cg1++) {
                            {
                                {
                                    __half2 _pk[4];
                                    _pk[0] = __floats2half2_rn(_tmem_load_35[state_vec_idx_cg1 * 8 + 0], _tmem_load_35[state_vec_idx_cg1 * 8 + 1]);
                                    _pk[1] = __floats2half2_rn(_tmem_load_35[state_vec_idx_cg1 * 8 + 2], _tmem_load_35[state_vec_idx_cg1 * 8 + 3]);
                                    _pk[2] = __floats2half2_rn(_tmem_load_35[state_vec_idx_cg1 * 8 + 4], _tmem_load_35[state_vec_idx_cg1 * 8 + 5]);
                                    _pk[3] = __floats2half2_rn(_tmem_load_35[state_vec_idx_cg1 * 8 + 6], _tmem_load_35[state_vec_idx_cg1 * 8 + 7]);
                                    *reinterpret_cast<uint4*>(&((__half*)(output_state + (state_base_idx_cg1 + (long long)(state_col_block_cg1 * 32) + (long long)(state_vec_idx_cg1 * 8))))[0 + 0]) = *reinterpret_cast<uint4*>(&_pk[0]);
                                }
                            }
                        }
                    }
                ...
                if (STORE_FINAL_STATE != 0 && USE_INITIAL_STATE != 0 && num_chunks_b_1 == 0) {
                    ...
                    #pragma unroll
                    for (int empty_state_vec = 0; empty_state_vec < 16; empty_state_vec++) {
                        int empty_state_col = empty_state_vec * 8;
                        float _vec_load_0[8];
                        {
                            const uint4* _vptr_13 = reinterpret_cast<const uint4*>(initial_state + empty_initial_base + (long long)empty_state_col);
                            uint4 _vld_13[1];
                            #pragma unroll
                        ...
                        {
                            __half2 _pk[4];
                            _pk[0] = __floats2half2_rn(_vec_load_0[0 + 0], _vec_load_0[0 + 1]);
                            _pk[1] = __floats2half2_rn(_vec_load_0[0 + 2], _vec_load_0[0 + 3]);
                            _pk[2] = __floats2half2_rn(_vec_load_0[0 + 4], _vec_load_0[0 + 5]);
                            _pk[3] = __floats2half2_rn(_vec_load_0[0 + 6], _vec_load_0[0 + 7]);
                            *reinterpret_cast<uint4*>(&((__half*)(output_state + (empty_output_base + (long long)empty_state_col)))[0]) = *reinterpret_cast<uint4*>(&_pk[0]);
                        }
                    }
                }
            }
