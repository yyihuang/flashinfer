...
kernel_cake_dsv4_d26fea2ca352792e0dd5(const __grid_constant__ CUtensorMap tmap_q, const __grid_constant__ CUtensorMap tmap_swa_kv, const __grid_constant__ CUtensorMap tmap_compressed_kv, __nv_bfloat16* __restrict__ O, float* __restrict__ partial_lse, int* __restrict__ swa_indices, int* __restrict__ compressed_indices, int* __restrict__ sparse_topk_lens, float* __restrict__ sinks, float* __restrict__ bmm1_scale, float* __restrict__ bmm2_scale, int num_heads, int swa_index_stride, int compressed_index_stride, int sparse_topk_lens_offset, int num_query_tokens, int sparse_topk, int has_sinks, int total_work_items)
{
    ...
    if (warp == 9) {
        ...
        if (leader) {
            ...
            // index_empty: 6 barriers, init_count=192
            mbarrier_init(smem + 128, 192);
            mbarrier_init(smem + 136, 192);
            mbarrier_init(smem + 144, 192);
            mbarrier_init(smem + 152, 192);
            mbarrier_init(smem + 160, 192);
            mbarrier_init(smem + 168, 192);
            asm volatile("fence.mbarrier_init.release.cluster;" ::: "memory");
        }
    }
    if (warp == 0) {
            ...
            for (unsigned int work_idx = 0; work_idx < 1; work_idx++) {
                ...
                for (int tile = 1; tile < num_kv_tiles; tile++) {
                    ...
                    int v_index_stage_base = smem_sparse_indices_addr + load_v_index_stage * 1024;
                    {
                        ...
                        if (lane < 16) {
                            ...
                            int group_2 = (load_warp_rank & 1) * 16 + lane;
                ...
                int global_last_tile = first_tile_1 + last_tile;
                {
                    ...
                    if (lane < 16) {
                        ...
                        int group_3 = (load_warp_rank & 1) * 16 + lane;
