            ...
            for (unsigned int work_slot = work_begin; work_slot < work_end; work_slot++) {
                ...
                for (int tile = 0; tile < num_kv_tiles; tile++) {
                    ...
                    if (valid_cols == 64 && ~(keep_lo & keep_hi) == 0) {
                        ...
                        float _max_6 = max_noftz(_max3_27, scores[63]);
                        float _max3_30;
                        #if defined(__CUDA_ARCH__) && (__CUDA_ARCH__ < 1000)
                        #error "Max3 requires PTX three-input max.f32 support on sm_100+"
                        #endif
                    ...
                    float _max_7 = max_noftz(tile_max, row_max_val);
                    float new_max = _max_7;
                    smem_softmax_exchange[exchange_idx] = new_max;
                    asm volatile("barrier.sync 3, 128;" ::: "memory");
                    float _max_8 = max_noftz(new_max, smem_softmax_exchange[exchange_idx ^ 64]);
                    new_max = _max_8;
                    asm volatile("barrier.sync 3, 128;" ::: "memory");
    ...
    if (warp >= 4 && warp <= 7) {
        ...
        { // correction_wg_main
            ...
            for (unsigned int work_slot_1 = work_begin_1; work_slot_1 < work_end_1; work_slot_1++) {
                ...
                {
                    int _max_9 = ((sparse_topk_lens[work_idx_1 / ((0) ? 3 : 1)] + sparse_topk_lens_offset) > (0) ? (sparse_topk_lens[work_idx_1 / ((0) ? 3 : 1)] + sparse_topk_lens_offset) : (0));
                    int _min_6 = ((_max_9) < (sparse_topk) ? (_max_9) : (sparse_topk));
