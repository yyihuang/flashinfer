...
// tvm-ffi direct-source launcher for Cake kernel 'kernel_cake_dsv4_62106177d2cf8accbaaf'.
#include <cuda.h>
#include <cuda_bf16.h>
#include <cuda_fp16.h>
...
extern "C" __global__ void kernel_cake_dsv4_62106177d2cf8accbaaf(const __grid_constant__ CUtensorMap tmap_q, const __grid_constant__ CUtensorMap tmap_swa_kv, const __grid_constant__ CUtensorMap tmap_compressed_kv, __nv_bfloat16* __restrict__ O, float* __restrict__ partial_lse, int* __restrict__ swa_indices, int* __restrict__ compressed_indices, int* __restrict__ sparse_topk_lens, int* __restrict__ seq_lens, int* __restrict__ cum_seq_lens_q, float* __restrict__ sinks, float* __restrict__ bmm1_scale, float* __restrict__ bmm2_scale, int num_heads, int swa_index_stride, int compressed_index_stride, int sparse_topk_lens_offset, int num_query_tokens, int sparse_topk, int has_sinks, int total_work_items, int max_q_len, int batch_size);


namespace cake_host_shim_c4a783a467b9acf5 {
...
void Run(TensorView arg_tmap_q, TensorView arg_tmap_swa_kv, TensorView arg_tmap_compressed_kv, TensorView arg_O, TensorView arg_partial_lse, TensorView arg_swa_indices, TensorView arg_compressed_indices, TensorView arg_sparse_topk_lens, TensorView arg_seq_lens, TensorView arg_cum_seq_lens_q, TensorView arg_sinks, TensorView arg_bmm1_scale, TensorView arg_bmm2_scale, int64_t arg_num_heads, int64_t arg_swa_index_stride, int64_t arg_compressed_index_stride, int64_t arg_sparse_topk_lens_offset, int64_t arg_num_query_tokens, int64_t arg_sparse_topk, int64_t arg_has_sinks, int64_t arg_total_work_items, int64_t arg_max_q_len, int64_t arg_batch_size, int64_t grid_x, int64_t grid_y, int64_t grid_z) {
    ...
    if (it == smem_status_by_device->end()) {
      ...
      if (smem_status == cudaSuccess) {
        smem_status = cudaFuncSetAttribute(
            reinterpret_cast<const void*>(kernel_cake_dsv4_62106177d2cf8accbaaf),
  ...
  TVM_FFI_CHECK(smem_status == cudaSuccess, RuntimeError)
      << "cudaFuncSetAttribute for kernel_cake_dsv4_62106177d2cf8accbaaf failed: "
  ...
  cudaError_t launch_status = cudaLaunchKernelExC(
      &config, reinterpret_cast<const void*>(kernel_cake_dsv4_62106177d2cf8accbaaf), kargs);
  TVM_FFI_CHECK(launch_status == cudaSuccess, RuntimeError)
      << "cudaLaunchKernelExC for kernel_cake_dsv4_62106177d2cf8accbaaf failed: "
      << cudaGetErrorString(launch_status);
}

}  // namespace cake_host_shim_c4a783a467b9acf5

TVM_FFI_DLL_EXPORT_TYPED_FUNC(run, cake_host_shim_c4a783a467b9acf5::Run);
