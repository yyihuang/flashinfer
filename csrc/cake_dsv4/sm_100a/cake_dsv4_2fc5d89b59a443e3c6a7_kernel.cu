...
kernel_cake_dsv4_2fc5d89b59a443e3c6a7(const __grid_constant__ CUtensorMap tmap_q, const __grid_constant__ CUtensorMap tmap_swa_kv, const __grid_constant__ CUtensorMap tmap_compressed_kv, __nv_bfloat16* __restrict__ partial_O, float* __restrict__ partial_lse, int* __restrict__ swa_indices, int* __restrict__ compressed_indices, int* __restrict__ sparse_topk_lens, float* __restrict__ sinks, float* __restrict__ bmm1_scale, float* __restrict__ bmm2_scale, int num_heads, int swa_index_stride, int compressed_index_stride, int sparse_topk_lens_offset, int sparse_topk, int num_splits, int has_sinks)
{
    ...
    if (warp >= 9 && warp <= 12) {
        { // load_warp_main
            ...
            int tile_col_base = split_idx_2 * 128;
            int* index_row = ((split_idx_2 == 0) ? swa_indices + (query_idx_2 * swa_index_stride) : compressed_indices + (query_idx_2 * compressed_index_stride + (tile_col_base - 128)));
            if (cta_rank == 0) {
                if (load_warp_rank == 0) {
                    if (elect_sync()) {
            ...
            int vector_group = load_warp_rank * 8 + lane;
            if (lane < 8) {
                ...
                if (global_vector_offset + 3 < sparse_topk) {
                    asm volatile("cp.async.ca.shared::cta.global [%0], [%1], 16;"
                        :: "r"(smem_indices_addr + (unsigned int)(vector_offset * 4)), "l"(index_row + vector_offset));
                } else {
                    int staged_rows[4];
                    #pragma unroll
                    for (int row_i = 0; row_i < 4; row_i++) {
                        ...
                        if (global_vector_offset + row_i < sparse_topk) {
                            staged_rows[row_i] = index_row[vector_offset + row_i];
                        }
                    }
