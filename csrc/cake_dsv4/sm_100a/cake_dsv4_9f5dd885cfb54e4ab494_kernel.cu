...
kernel_cake_dsv4_9f5dd885cfb54e4ab494(const __grid_constant__ CUtensorMap tmap_q, const __grid_constant__ CUtensorMap tmap_swa_kv, const __grid_constant__ CUtensorMap tmap_compressed_kv, __nv_bfloat16* __restrict__ partial_O, float* __restrict__ partial_lse, __nv_bfloat16* __restrict__ O, unsigned int* __restrict__ partition_arrivals, int* __restrict__ swa_indices, int* __restrict__ compressed_indices, int* __restrict__ sparse_topk_lens, float* __restrict__ sinks, float* __restrict__ bmm1_scale, float* __restrict__ bmm2_scale, int num_heads, int swa_index_stride, int compressed_index_stride, int sparse_topk_lens_offset, int sparse_topk, int num_splits, int num_head_tiles, int has_sinks)
{
    ...
    if (warp <= 3) {
        ...
        { // softmax_main
            ...
            int _max_1 = ((sparse_topk_lens[query_idx] + sparse_topk_lens_offset) > (0) ? (sparse_topk_lens[query_idx] + sparse_topk_lens_offset) : (0));
            int _min_1 = ((_max_1) < (sparse_topk) ? (_max_1) : (sparse_topk));
            int active_topk = _min_1;
            ...
            int score_addr = taddr + (unsigned int)(warp * 32 << 16);
            ...
            if (lane < 8) {
                int h_lane = lane;
                float _max_2 = max_noftz(smem_exch[h_lane], smem_exch[8 + h_lane]);
                float _max_3 = max_noftz(smem_exch[16 + h_lane], smem_exch[24 + h_lane]);
                float _max_4 = max_noftz(_max_2, _max_3);
                tile_max[h_lane] = _max_4;
            }
            #pragma unroll
            ...
            for (int h_4 = 0; h_4 < 8; h_4++) {
                ...
                if (has_sinks != 0 && split_idx == 0 && global_head < num_heads) {
                    ...
                    float _max_5 = max_noftz(row_max, sink_unscaled);
                    row_max = _max_5;
                }
    ...
    if (warp >= 4 && warp <= 7) {
        ...
        { // correction_lo_main
            ...
            const int d_lane = local_warp * 32 + lane;
                ...
                if (warp == 4) {
                    if (elect_sync()) {
                        ...
                        asm volatile("atom.acq_rel.gpu.global.add.u32 %0, [%1], %2;"
                            : "=r"(_atomic_old_0) : "l"(&partition_arrivals[merge_group]), "r"(static_cast<uint32_t>(1)) : "memory");
                        unsigned int prior_arrivals = _atomic_old_0;
                        smem_exch[0] = __uint_as_float(prior_arrivals);
                    }
                }
                asm volatile("barrier.sync 10, 128;" ::: "memory");
                float flag_bits = smem_exch[0];
                int prior_count = __as_u32(flag_bits);
                int last_split = num_splits - 1;
                if (prior_count == last_split) {
                    #pragma unroll
                    for (int local_h = 0; local_h < 2; local_h++) {
                        ...
                        if (global_head_4 < num_heads) {
                            ...
                            int lane_d = lane * 8;
                            int _min_2 = ((1) < (last_split) ? (1) : (last_split));
                            int split_1 = _min_2;
                            int _min_3 = ((2) < (last_split) ? (2) : (last_split));
                            int split_2 = _min_3;
                            float _vec_load_0[8];
                            {
                            ...
                            float _vec_load_1[8];
                            {
                                const uint4* _vptr_1 = reinterpret_cast<const uint4*>(partial_O + (partial_base + split_1 * 512 + lane_d) + 0);
                                uint4 _vld_1[1];
                                #pragma unroll
                                for (int _blk = 0; _blk < 1; _blk++) {
                            ...
                            float _vec_load_2[8];
                            {
                                const uint4* _vptr_2 = reinterpret_cast<const uint4*>(partial_O + (partial_base + split_2 * 512 + lane_d) + 0);
                                uint4 _vld_2[1];
                                #pragma unroll
                                for (int _blk = 0; _blk < 1; _blk++) {
                            ...
                            float _vec_load_3[8];
                            {
                                const uint4* _vptr_3 = reinterpret_cast<const uint4*>(partial_O + (partial_base + 256 + lane_d) + 0);
                                uint4 _vld_3[1];
                                #pragma unroll
                                for (int _blk = 0; _blk < 1; _blk++) {
                            ...
                            float _vec_load_4[8];
                            {
                                const uint4* _vptr_4 = reinterpret_cast<const uint4*>(partial_O + (partial_base + split_1 * 512 + 256 + lane_d) + 0);
                                uint4 _vld_4[1];
                                #pragma unroll
                                for (int _blk = 0; _blk < 1; _blk++) {
                            ...
                            float _vec_load_5[8];
                            {
                                const uint4* _vptr_5 = reinterpret_cast<const uint4*>(partial_O + (partial_base + split_2 * 512 + 256 + lane_d) + 0);
                                uint4 _vld_5[1];
                                #pragma unroll
                                for (int _blk = 0; _blk < 1; _blk++) {
                            ...
                            if (lane < num_splits) {
                                local_lse = partial_lse[lse_base + lane];
                            }
                            ...
                            float _shfl_1;
                            asm volatile("shfl.sync.idx.b32 %0, %1, %2, 0x1f, 0xffffffff;" : "=f"(_shfl_1) : "f"(normalized_weight), "r"(0));
                            float weight_0 = _shfl_1;
                            float _shfl_2;
                            asm volatile("shfl.sync.idx.b32 %0, %1, %2, 0x1f, 0xffffffff;" : "=f"(_shfl_2) : "f"(normalized_weight), "r"(1));
                            float weight_1 = _shfl_2;
                            float _shfl_3;
                            asm volatile("shfl.sync.idx.b32 %0, %1, %2, 0x1f, 0xffffffff;" : "=f"(_shfl_3) : "f"(normalized_weight), "r"(2));
                            float weight_2 = _shfl_3;
                            float merged_0[8];
                            float merged_1[8];
                            #pragma unroll
                            for (int elem = 0; elem < 8; elem++) {
                                merged_0[elem] = weight_0 * _vec_load_0[elem] + weight_1 * _vec_load_1[elem] + weight_2 * _vec_load_2[elem];
                                merged_1[elem] = weight_0 * _vec_load_3[elem] + weight_1 * _vec_load_4[elem] + weight_2 * _vec_load_5[elem];
                            }
                            int output_base = (query_idx_1 * num_heads + global_head_4) * 512 + lane_d;
                            {
                                __nv_bfloat162 _pk[4];
                                _pk[0] = __floats2bfloat162_rn(merged_0[0 + 0], merged_0[0 + 1]);
                                _pk[1] = __floats2bfloat162_rn(merged_0[0 + 2], merged_0[0 + 3]);
                                _pk[2] = __floats2bfloat162_rn(merged_0[0 + 4], merged_0[0 + 5]);
                                _pk[3] = __floats2bfloat162_rn(merged_0[0 + 6], merged_0[0 + 7]);
                                *reinterpret_cast<uint4*>(&((__nv_bfloat16*)(O + output_base))[0]) = *reinterpret_cast<uint4*>(&_pk[0]);
                            }
                            {
                                __nv_bfloat162 _pk[4];
                                _pk[0] = __floats2bfloat162_rn(merged_1[0 + 0], merged_1[0 + 1]);
                                _pk[1] = __floats2bfloat162_rn(merged_1[0 + 2], merged_1[0 + 3]);
                                _pk[2] = __floats2bfloat162_rn(merged_1[0 + 4], merged_1[0 + 5]);
                                _pk[3] = __floats2bfloat162_rn(merged_1[0 + 6], merged_1[0 + 7]);
                    ...
                    if (warp == 4) {
                        if (elect_sync()) {
                            {
                                unsigned int* _gcr_p = reinterpret_cast<unsigned int*>(partition_arrivals) + (merge_group);
    ...
    if (warp >= 8 && warp <= 11) {
        ...
        { // correction_hi_main
            ...
            const int d_lane_1 = local_warp_1 * 32 + lane;
    ...
    if (warp >= 13 && warp <= 15) {
        { // load_warp_main
            ...
            int tile_col_base = split_idx_3 * 128;
            int* row_ptr = ((tile_col_base < 128) ? swa_indices + (query_idx_3 * swa_index_stride) : compressed_indices + (query_idx_3 * compressed_index_stride + (tile_col_base - 128)));
            ...
            if (load_rank == 0) {
                ...
                int global_vector_offset = tile_col_base + vector_offset;
                if (global_vector_offset + 3 < sparse_topk) {
                    asm volatile("cp.async.ca.shared::cta.global [%0], [%1], 16;"
                        :: "r"(smem_indices_addr + (unsigned int)(vector_offset * 4)), "l"(row_ptr + vector_offset));
                } else {
                    int staged_rows[4];
                    #pragma unroll
                    for (int row_i = 0; row_i < 4; row_i++) {
                        ...
                        if (global_vector_offset + row_i < sparse_topk) {
                            staged_rows[row_i] = row_ptr[vector_offset + row_i];
                        }
                    }
            ...
            int loader_thread = load_rank * 32 + lane;
            ...
            for (int k_stage_1 = 0; k_stage_1 < 4; k_stage_1++) {
                if (k_stage_1 % 3 == load_rank) {
                    ...
                    if (valid_groups > lane) {
                        int dst_k = smem_kv_addr + (unsigned int)(k_stage_1 * 32768);
                        if (split_idx_3 == 0) {
                            tma_gather4_gmem2smem(dst_k + lane * 512, (&tmap_swa_kv), k_stage_1 * 128, row0, row1, row2, row3, kv_full_addr + (k_stage_1) * 8);
                            tma_gather4_gmem2smem(dst_k + 16384 + lane * 512, (&tmap_swa_kv), k_stage_1 * 128 + 64, row0, row1, row2, row3, kv_full_addr + (k_stage_1) * 8);
                        } else {
                            tma_gather4_gmem2smem(dst_k + lane * 512, (&tmap_compressed_kv), k_stage_1 * 128, row0, row1, row2, row3, kv_full_addr + (k_stage_1) * 8);
                            tma_gather4_gmem2smem(dst_k + 16384 + lane * 512, (&tmap_compressed_kv), k_stage_1 * 128 + 64, row0, row1, row2, row3, kv_full_addr + (k_stage_1) * 8);
                        }
                    }
                }
            }
