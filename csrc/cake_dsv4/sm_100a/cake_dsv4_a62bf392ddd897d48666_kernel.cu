        ...
        { // softmax_main
            ...
            for (int tile = 0; tile < 1; tile++) {
                if (tile_count > tile) {
                    ...
                    int* index_row = swa_indices + (query_idx * swa_index_stride + tile * 128 + col_half * 64);
                    #pragma unroll
                    for (int chunk = 0; chunk < 64; chunk += 4) {
                        if (valid_cols > chunk) {
                            int _vec_load_1[4];
                            {
                                const int4* _ivptr_2 = reinterpret_cast<const int4*>(index_row + chunk + 0);
                                int4 _ivld_2;
                                _ivld_2 = *_ivptr_2;
                                _vec_load_1[0 + 0] = _ivld_2.x;
                                _vec_load_1[0 + 1] = _ivld_2.y;
                                _vec_load_1[0 + 2] = _ivld_2.z;
                                _vec_load_1[0 + 3] = _ivld_2.w;
                            }
                            #pragma unroll
                            for (int i = 0; i < 4; i++) {
                                if (_vec_load_1[i] < 0) {
                ...
                for (int v_stage = 0; v_stage < 2; v_stage++) {
                    ...
                    if (my_row < num_heads) {
                        ...
                        for (int offset = 0; offset < 64; offset += 8) {
                            {
                                const float2 _prescale2_7 = {inv_sum * output_scale, inv_sum * output_scale};
                                #if __CUDA_ARCH__ >= 1000
                                #pragma unroll
                                for (int _ps = 0; _ps < 4; _ps++)
                                    mul_f32x2_inplace(&reinterpret_cast<float2*>(&_tmem_load_1[offset])[_ps], _prescale2_7);
                                #else
                                #pragma unroll
    ...
    if (warp >= 5 && warp <= 7) {
        { // load_warp_main
            ...
            if (group < 32) {
                ...
                int* index_row_1 = swa_indices + (query_idx_2 * swa_index_stride);
                int raw_rows[4];
                if (index_base + 3 < sparse_topk) {
                    int _vec_load_0[4];
                    {
                        const int4* _ivptr_0 = reinterpret_cast<const int4*>(index_row_1 + index_base + 0);
                        int4 _ivld_0;
                        _ivld_0 = *_ivptr_0;
                    ...
                    for (int i_1 = 0; i_1 < 4; i_1++) {
                        raw_rows[i_1] = _vec_load_0[i_1];
                    }
                } else {
                    #pragma unroll
                    for (int i_2 = 0; i_2 < 4; i_2++) {
                        raw_rows[i_2] = -1;
                        if (index_base + i_2 < sparse_topk) {
                            raw_rows[i_2] = index_row_1[index_base + i_2];
                        }
                    }
                }
