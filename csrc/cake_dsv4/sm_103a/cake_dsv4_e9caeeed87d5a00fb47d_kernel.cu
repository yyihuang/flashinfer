...
kernel_cake_dsv4_e9caeeed87d5a00fb47d(CakeTensorMap const* tmap_q, CakeTensorMap const* tmap_swa_kv, CakeTensorMap const* tmap_compressed_kv, __nv_bfloat16* __restrict__ O, int* __restrict__ swa_indices, int* __restrict__ compressed_indices, int* __restrict__ sparse_topk_lens, float* __restrict__ sinks, float* __restrict__ bmm1_scale, float* __restrict__ bmm2_scale, int num_heads, int swa_index_stride, int compressed_index_stride, int sparse_topk_lens_offset, int sparse_topk, int num_query_tokens, int has_sinks)
{
    ...
    if (warp <= 3) {
        ...
        { // softmax_main
            ...
            #pragma unroll 1
            for (unsigned int work_iter = 0; work_iter < num_query_tokens; work_iter++) {
                if (query_idx >= (unsigned int)num_query_tokens) {
                    break;
                }
    ...
    if (warp >= 4 && warp <= 7) {
        ...
        { // correction_main
            ...
            #pragma unroll 1
            for (unsigned int work_iter_1 = 0; work_iter_1 < num_query_tokens; work_iter_1++) {
                if (query_idx_1 >= (unsigned int)num_query_tokens) {
                    break;
                }
    ...
    if (warp == 8) {
        { // mma_warp_main
            ...
            #pragma unroll 1
            for (unsigned int work_iter_2 = 0; work_iter_2 < num_query_tokens; work_iter_2++) {
                if (query_idx_2 >= (unsigned int)num_query_tokens) {
                    break;
                }
    ...
    if (warp == 9) {
        { // page_warp_main
            ...
            #pragma unroll 1
            for (unsigned int work_iter_3 = 0; work_iter_3 < num_query_tokens; work_iter_3++) {
                ...
                for (int pair = 0; pair < num_page_pairs; pair++) {
                    ...
                    for (int duplicate = 0; duplicate < 2; duplicate++) {
                        ...
                        asm volatile("cp.async.ca.shared::cta.global [%0], [%1], 16;"
                            :: "r"(smem_page_offsets_addr + page_stage * 1024 + (unsigned int)((128 + lane_base) * 4)), "l"(src1));
                        asm volatile(
                            "{\n\t"
    ...
    if (warp == 10) {
        { // scheduler_warp_main
            ...
            #pragma unroll 1
            for (unsigned int work_iter_4 = 0; work_iter_4 < num_query_tokens; work_iter_4++) {
                if (query_idx_4 >= (unsigned int)num_query_tokens) {
                    break;
                }
    ...
    if (warp == 11) {
        { // padding_warp_main
            ...
            #pragma unroll 1
            for (unsigned int work_iter_5 = 0; work_iter_5 < num_query_tokens; work_iter_5++) {
                if (query_idx_5 >= (unsigned int)num_query_tokens) {
                    break;
                }
    ...
    if (warp >= 12 && warp <= 15) {
        ...
        { // load_warp_main
            ...
            #pragma unroll 1
            for (unsigned int work_iter_6 = 0; work_iter_6 < num_query_tokens; work_iter_6++) {
                if (query_idx_6 >= (unsigned int)num_query_tokens) {
                    break;
                }
                int _max_0 = ((sparse_topk_lens[query_idx_6] + sparse_topk_lens_offset) > (0) ? (sparse_topk_lens[query_idx_6] + sparse_topk_lens_offset) : (0));
