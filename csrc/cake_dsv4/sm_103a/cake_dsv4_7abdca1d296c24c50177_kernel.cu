...
kernel_cake_dsv4_7abdca1d296c24c50177(CakeTensorMap const* tmap_q, CakeTensorMap const* tmap_swa_kv, CakeTensorMap const* tmap_compressed_kv, __nv_bfloat16* __restrict__ O, int* __restrict__ swa_indices, int* __restrict__ compressed_indices, int* __restrict__ sparse_topk_lens, int* __restrict__ seq_lens, int* __restrict__ cum_seq_lens_q, float* __restrict__ sinks, float* __restrict__ bmm1_scale, float* __restrict__ bmm2_scale, int swa_index_stride, int compressed_index_stride, int sparse_topk_lens_offset, int sparse_topk, int max_q_len, int batch_size, int has_sinks)
{
    ...
    if (warp <= 3) {
        ...
        { // softmax_main
            ...
            const int ldtm_row_base = warp_in_wg * 32 + lane / 4;
            ...
            for (unsigned int _work = 0; _work < 120; _work++) {
                ...
                for (int tile = 0; tile < num_steps; tile++) {
                    ...
                    float pair_max[2];
                    float _max_5 = max_noftz(scores[0], scores[2]);
                    float _max_6 = max_noftz(scores[4], scores[6]);
                    float _max_7 = max_noftz(_max_5, _max_6);
                    pair_max[0] = _max_7;
                    float _max_8 = max_noftz(scores[1], scores[3]);
                    ...
                    for (int c_2 = 0; c_2 < 2; c_2++) {
                        ...
                        float _max_11 = max_noftz(pair_max[c_2], _shfl_xor_0);
                        pair_max[c_2] = _max_11;
                        float _shfl_xor_1 = __shfl_xor_sync(0xFFFFFFFF, pair_max[c_2], 8);
                        float _max_12 = max_noftz(pair_max[c_2], _shfl_xor_1);
                        pair_max[c_2] = _max_12;
                    }
                    ...
                    for (int c_3 = 0; c_3 < 2; c_3++) {
                        ...
                        float _max_13 = max_noftz(row_max_pair[c_3], pair_max[c_3]);
                        new_max_pair[c_3] = _max_13;
                    }
                    if (lane < 8) {
    ...
    if (warp >= 4 && warp <= 7) {
        ...
        { // correction_main
            ...
            const int corr_tid = corr_rank * 32 + lane;
            ...
            for (unsigned int _work_1 = 0; _work_1 < 120; _work_1++) {
                ...
                if (work_x_1 < (unsigned int)q_len_1) {
                    ...
                    int _min_4 = ((_max_14) < (sparse_topk) ? (_max_14) : (sparse_topk));
                    active_topk_1 = _min_4;
                }
        ...
        { // page_loader_main
            ...
            for (unsigned int _work_3 = 0; _work_3 < 120; _work_3++) {
                ...
                if (work_x_3 < (unsigned int)q_len_3) {
                    ...
                    active_topk_3 = _min_0;
                }
                int num_steps_3 = (active_topk_3 + 128 - 1) / 128;
                int num_pairs = (num_steps_3 + 1) / 2;
                int _max_1 = ((active_topk_3 - 1 & -4) > (0) ? (active_topk_3 - 1 & -4) : (0));
                int last_aligned = _max_1;
