...
// tvm-ffi direct-source launcher for Cake kernel 'kernel_cake_dsv4_f46b148e7e22b94153ca'.
#include <cuda.h>
#include <cuda_bf16.h>
#include <cuda_fp16.h>
...
void Run(TensorView arg_tmap_q, TensorView arg_tmap_swa_kv, TensorView arg_tmap_compressed_kv, TensorView arg_O, TensorView arg_swa_indices, TensorView arg_compressed_indices, TensorView arg_sparse_topk_lens, TensorView arg_seq_lens, TensorView arg_cum_seq_lens_q, TensorView arg_sinks, TensorView arg_bmm1_scale, TensorView arg_bmm2_scale, int64_t arg_num_heads, int64_t arg_swa_index_stride, int64_t arg_compressed_index_stride, int64_t arg_sparse_topk_lens_offset, int64_t arg_sparse_topk, int64_t arg_batch_size, int64_t arg_max_q_len, int64_t arg_ragged_query, int64_t arg_has_sinks, tvm::ffi::Tensor arg_tma_descriptor_workspace, int64_t grid_x, int64_t grid_y, int64_t grid_z) {
    ...
    if (it == smem_status_by_device->end()) {
      ...
      if (smem_status == cudaSuccess) {
        smem_status = cudaFuncSetAttribute(
            reinterpret_cast<const void*>(kernel_cake_dsv4_f46b148e7e22b94153ca),
  ...
  TVM_FFI_CHECK(smem_status == cudaSuccess, RuntimeError)
      << "cudaFuncSetAttribute for kernel_cake_dsv4_f46b148e7e22b94153ca failed: "
  ...
  cudaError_t launch_status = cudaLaunchKernelExC(
      &config, reinterpret_cast<const void*>(kernel_cake_dsv4_f46b148e7e22b94153ca), kargs);
  TVM_FFI_CHECK(launch_status == cudaSuccess, RuntimeError)
      << "cudaLaunchKernelExC for kernel_cake_dsv4_f46b148e7e22b94153ca failed: "
      << cudaGetErrorString(launch_status);
}

}  // namespace cake_host_shim_5a81bc4b7e01f74f

TVM_FFI_DLL_EXPORT_TYPED_FUNC(run, cake_host_shim_5a81bc4b7e01f74f::Run);
