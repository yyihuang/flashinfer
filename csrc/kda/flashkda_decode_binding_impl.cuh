...
static_assert(SMEM_TOTAL == 42368);
...
void Run(TensorView q, TensorView k, TensorView v, TensorView g, TensorView beta, TensorView A_log,
  ...
  const LaunchContext ctx = CheckInputs<HEAD_DIM, ACTIVE_T, GATE_KIND>(
  ...
  constexpr int32_t kBlockThreads = HEAD_DIM == 64 ? 128 : THREADS;
  const dim3 grid(ctx.num_value_heads, ctx.num_sequences, 1);
