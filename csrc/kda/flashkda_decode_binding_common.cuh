...
inline void CheckStateLayout(const TensorView& state, int64_t num_value_heads, int64_t head_dim) {
  ...
  TVM_FFI_ICHECK(state.size(0) > 0 && state.size(1) == num_value_heads &&
      ...
      << "initial_state must have shape [slots, HV, D, D]";
  TVM_FFI_ICHECK(state.stride(3) == 1 && state.stride(2) == head_dim &&
      ...
      << "initial_state must have compact [HV, D, D] blocks and may only pad "
         "the outer slot stride";
...
template <int kHeadDim, int kTokens, int kGateKind>
LaunchContext CheckInputs(const TensorView& q, const TensorView& k, const TensorView& v,
  ...
  TVM_FFI_ICHECK(q.ndim() == 4 && q.size(0) == 1 && q.size(3) == kHeadDim)
      << "q must have shape [1, total_tokens, H, D]";
  ...
  TVM_FFI_ICHECK(v.ndim() == 4 && v.size(0) == 1 && v.size(1) == total_tokens &&
                 v.size(3) == kHeadDim)
      << "v must have shape [1, total_tokens, HV, D]";
  ...
  TVM_FFI_ICHECK(total_tokens == num_sequences * kTokens)
      << "packed speculative input must contain exactly N * T tokens";
  TVM_FFI_ICHECK(ssm_state_indices.numel() == num_sequences * kTokens)
      << "ssm_state_indices must contain N * T elements";
  ...
  if constexpr (kGateKind == 0) {
    ...
    TVM_FFI_ICHECK(A_log.ndim() == 1 && A_log.numel() == num_heads)
        << "A_log must be contiguous rank-1 [H]";
    TVM_FFI_ICHECK(dt_bias.ndim() == 1 && dt_bias.numel() == num_heads * kHeadDim)
        << "dt_bias must be contiguous rank-1 [H * D]";
    ...
    TVM_FFI_ICHECK(std::isfinite(lower_bound) && std::isfinite(static_cast<float>(lower_bound)) &&
                   lower_bound < 0.0)
        << "lower-bound variants require a finite negative lower_bound";
  }
