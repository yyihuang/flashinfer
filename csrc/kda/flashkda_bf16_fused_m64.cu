...
// clang-format off
// Generated by the Cake Loom source exporter (device kernel).
// Provenance: generated Loom schedule 'flashkda_bf16_fused_m64' at Cake 209b1d1bf2dc48e39a865f34f7dcdb7f742bb0c2.
// Raw generated body SHA256: 29ae7194a13203b42edb3fddf067fb2c5893f33c7124b1389f4a991a20c4fb60
// The sm_100a and sm_103a generated bodies are byte-identical.
// Local WIP: correctness/export review only; performance promotion is pending.
...
#define SMEM_SMEM_KR_LAST_ROW_SCRATCH_OFF 42632
#define SMEM_SMEM_KR_LAST_ROW_SCRATCH_STAGE_BYTES 256
#define SMEM_SMEM_KR_LAST_ROW_SCRATCH_STRIDE 41984
...
// FLASHINFER INTEGRATION END: allow exact state alias
{
    ...
    unsigned int* smem_kr_last_row_scratch = reinterpret_cast<unsigned int*>(smem_raw + 42632);
    const int smem_kr_last_row_scratch_addr = smem + 42632;
        ...
        { // epilogue_main
            ...
            for (int chunk_idx_1 = 0; chunk_idx_1 < num_chunks_1; chunk_idx_1++) {
                ...
                if (chunk_is_full != 0) {
                    ...
                    int valid_tokens = seq_len_1 - chunk_idx_1 * 32;
                    long long out_base = ((bos_1 + (long long)(chunk_idx_1 * 32)) * (long long)num_heads + (long long)head_idx_1) * 128 + (long long)value_row_offset_1;
                    long long out_stride = (long long)num_heads * 128;
                    #pragma unroll
                    for (int token_group_2 = 0; token_group_2 < 4; token_group_2++) {
                        ...
                        if (token_pair_2 < valid_tokens) {
                            long long token_base_0 = out_base + (long long)token_pair_2 * out_stride;
                            out[token_base_0 + (long long)local_row_top_1] = _tmem_load_5[out_reg_base];
                            out[token_base_0 + (long long)local_row_bot_1] = _tmem_load_5[out_reg_base + 2];
                        }
                        if (valid_tokens > token_pair_2 + 1) {
                            long long token_base_1 = out_base + (long long)(token_pair_2 + 1) * out_stride;
                            out[token_base_1 + (long long)local_row_top_1] = _tmem_load_5[out_reg_base + 1];
                            out[token_base_1 + (long long)local_row_bot_1] = _tmem_load_5[out_reg_base + 3];
                        }
                    }
                }
                epilogue_stage += 1;
        ...
        { // prep_main
            ...
            for (int prep_iter = 0; prep_iter < num_prep_iters; prep_iter++) {
                ...
                if (prep_tid < 128) {
                    ...
                    float _exp2_0 = approx_exp2(prefix_log2 - lower_bound * 1.4426950408889634f * 16.0f);
                    smem_restore_factor_all[stage_f32 + gate_col] = _exp2_0;
                }
                if (prep_instance == 0) {
                    asm volatile("barrier.sync 11, 128;" ::: "memory");
                } else if (prep_instance == 1) {
                ...
                int _shfl_xor_0 = __shfl_xor_sync(0xFFFFFFFF, prep_tid, 1);
                int late_input_tid = _shfl_xor_0 ^ 1;
                #pragma unroll 1
                for (int work_pass = 0; work_pass < 4; work_pass++) {
                    int work_item = work_pass * 128 + late_input_tid;
                    ...
                    float _shfl_xor_1 = __shfl_xor_sync(0xFFFFFFFF, q_sum, 8);
                    q_sum += _shfl_xor_1;
                    float _shfl_xor_2 = __shfl_xor_sync(0xFFFFFFFF, k_sum, 8);
                    k_sum += _shfl_xor_2;
                    float _shfl_xor_3 = __shfl_xor_sync(0xFFFFFFFF, q_sum, 4);
                    q_sum += _shfl_xor_3;
                    float _shfl_xor_4 = __shfl_xor_sync(0xFFFFFFFF, k_sum, 4);
                    k_sum += _shfl_xor_4;
                    float _shfl_xor_5 = __shfl_xor_sync(0xFFFFFFFF, q_sum, 2);
                    q_sum += _shfl_xor_5;
                    float _shfl_xor_6 = __shfl_xor_sync(0xFFFFFFFF, k_sum, 2);
                    k_sum += _shfl_xor_6;
                    float _shfl_xor_7 = __shfl_xor_sync(0xFFFFFFFF, q_sum, 1);
                    q_sum += _shfl_xor_7;
                    float _shfl_xor_8 = __shfl_xor_sync(0xFFFFFFFF, k_sum, 1);
                    k_sum += _shfl_xor_8;
                    ...
                    int _shfl_xor_9 = __shfl_xor_sync(0xFFFFFFFF, lane, 1);
                    int late_segment = (_shfl_xor_9 ^ 1) & 15;
                    float qd_vec[8];
                    float kd_vec[8];
                    float ki_vec[8];
                    for (int elem_in_segment_1 = 0; elem_in_segment_1 < 8; elem_in_segment_1++) {
                        int col = late_segment * 8 + elem_in_segment_1;
                    ...
                    for (int word = 0; word < 4; word++) {
                        asm volatile("st.shared.b32 [%0], %1;" :: "r"((smem_kd_addr + prep_stage * 41984 + (unsigned int)(late_segment * 8 / 64 * 4096 + row_1 * 128 + late_segment * 8 % 64 * 2 ^ (late_segment * 8 / 64 * 4096 + row_1 * 128 + late_segment * 8 % 64 * 2 >> 7 & 7) << 4)) + (unsigned int)(word * 4)), "r"((packed_1[word])));
                    }
                    ...
                    for (int word_1 = 0; word_1 < 4; word_1++) {
                        asm volatile("st.shared.b32 [%0], %1;" :: "r"((smem_ki_addr + prep_stage * 41984 + (unsigned int)(late_segment * 8 / 64 * 4096 + row_1 * 128 + late_segment * 8 % 64 * 2 ^ (late_segment * 8 / 64 * 4096 + row_1 * 128 + late_segment * 8 % 64 * 2 >> 7 & 7) << 4)) + (unsigned int)(word_1 * 4)), "r"((packed_0_1[word_1])));
                    }
                    for (int elem_in_segment_2 = 0; elem_in_segment_2 < 8; elem_in_segment_2++) {
                        int col_1 = late_segment * 8 + elem_in_segment_2;
                        __nv_bfloat16 _cvt_bf16_0 = __float2bfloat16(k_raw_vec[elem_in_segment_2]);
                        float _cvt_f32_2 = __bfloat162float(_cvt_bf16_0);
                        float kr_numerator = _cvt_f32_2 * smem_restore_factor_all[stage_f32 + col_1];
                        float _div_full_0;
                        asm volatile("div.full.f32 %0, %1, %2;" : "=f"(_div_full_0) : "f"(kr_numerator), "f"(qd_vec[elem_in_segment_2]));
                        k_raw_vec[elem_in_segment_2] = _div_full_0;
                    }
                    #pragma unroll
                    for (int _ls = 0; _ls < 4; _ls++)
                    ...
                    for (int word_2 = 0; word_2 < 4; word_2++) {
                        asm volatile("st.shared.b32 [%0], %1;" :: "r"((smem_qd_addr + prep_stage * 41984 + (unsigned int)(late_segment * 8 / 64 * 4096 + row_1 * 128 + late_segment * 8 % 64 * 2 ^ (late_segment * 8 / 64 * 4096 + row_1 * 128 + late_segment * 8 % 64 * 2 >> 7 & 7) << 4)) + (unsigned int)(word_2 * 4)), "r"((packed_1_1[word_2])));
                    }
                    int sparse_store_base = smem_gate_addr + prep_stage * 41984 + (unsigned int)((row_1 * 128 + late_segment * 8) * 4);
                    int sparse_store_stride = 8;
                    if (row_1 == 31) {
                        sparse_store_base = smem_gate_addr + prep_stage * 41984 + (unsigned int)(17032 + late_segment * 16);
                        sparse_store_stride = 4;
                    }
                    ...
                    int sparse_addr = sparse_store_base;
                    asm volatile("st.shared.b32 [%0], %1;" :: "r"(sparse_addr), "r"((packed_pair[0])));
                    unsigned int packed_pair_2[1];
                    #pragma unroll
                    for (int _lp = 0; _lp < 1; _lp++) {
                        __nv_bfloat162 _bf2 = __float22bfloat162_rn(make_float2(k_raw_vec[_lp*2 + 2], k_raw_vec[_lp*2+1 + 2]));
                        packed_pair_2[_lp] = *(uint32_t*)&_bf2;
                    }
                    int sparse_addr_3 = sparse_store_base + sparse_store_stride;
                    asm volatile("st.shared.b32 [%0], %1;" :: "r"(sparse_addr_3), "r"((packed_pair_2[0])));
                    unsigned int packed_pair_4[1];
                    #pragma unroll
                    for (int _lp = 0; _lp < 1; _lp++) {
                        __nv_bfloat162 _bf2 = __float22bfloat162_rn(make_float2(k_raw_vec[_lp*2 + 4], k_raw_vec[_lp*2+1 + 4]));
                        packed_pair_4[_lp] = *(uint32_t*)&_bf2;
                    }
                    int sparse_addr_5 = sparse_store_base + 2 * sparse_store_stride;
                    asm volatile("st.shared.b32 [%0], %1;" :: "r"(sparse_addr_5), "r"((packed_pair_4[0])));
                    unsigned int packed_pair_6[1];
                    #pragma unroll
                    for (int _lp = 0; _lp < 1; _lp++) {
                        __nv_bfloat162 _bf2 = __float22bfloat162_rn(make_float2(k_raw_vec[_lp*2 + 6], k_raw_vec[_lp*2+1 + 6]));
                        packed_pair_6[_lp] = *(uint32_t*)&_bf2;
                    }
                    int sparse_addr_7 = sparse_store_base + 3 * sparse_store_stride;
                    asm volatile("st.shared.b32 [%0], %1;" :: "r"(sparse_addr_7), "r"((packed_pair_6[0])));
                }
                if (prep_instance == 0) {
                    asm volatile("barrier.sync 11, 128;" ::: "memory");
                } else if (prep_instance == 1) {
                ...
                int _shfl_xor_10 = __shfl_xor_sync(0xFFFFFFFF, prep_tid, 1);
                int late_backup_tid = _shfl_xor_10 ^ 1;
                if (late_backup_tid < 64) {
                    int backup_row = late_backup_tid / 8;
                    int backup_slot = late_backup_tid % 8;
                    int collision_half = backup_row % 2 * 2;
                    int backup_segment = backup_slot / 2 * 4 + collision_half + backup_slot % 2;
                    int backup_src = smem_gate_addr + prep_stage * 41984 + (unsigned int)((backup_row * 128 + backup_segment * 8) * 4);
                    int backup_dst = smem_gate_addr + prep_stage * 41984 + (unsigned int)((1024 + late_backup_tid * 8 + 1) * 4);
                    for (int backup_word = 0; backup_word < 4; backup_word++) {
                        unsigned int backup_pair[1];
                        asm volatile("ld.shared.b32 %0, [%1];" : "=r"(*reinterpret_cast<uint32_t*>(&backup_pair[0])) : "r"(backup_src + backup_word * 8));
                        asm volatile("st.shared.b32 [%0], %1;" :: "r"(backup_dst + backup_word * 8), "r"((backup_pair[0])));
                    }
                }
                for (int seed_backup_pass = 0; seed_backup_pass < 2; seed_backup_pass++) {
                    int seed_candidate = seed_backup_pass * 128 + late_backup_tid;
                    if (seed_candidate < 144) {
                        int seed_row = 13 + seed_candidate / 16;
                        int seed_segment = seed_candidate % 16;
                        int config = 0;
                        if (seed_row == 13) {
                            config = 8464;
                        } else if (seed_row == 14) {
                        ...
                        int seed_config = config;
                        int seed_mask = seed_config & 65535;
                        int seed_prefix = seed_config >> 16;
                        int seed_bit = 1 << seed_segment;
                        if ((seed_mask & seed_bit) != 0) {
                            int _popc_0 = __popc(seed_mask & seed_bit - 1);
                            int seed_item = seed_prefix + _popc_0;
                            int seed_backup_src = smem_gate_addr + prep_stage * 41984 + (unsigned int)((seed_row * 128 + seed_segment * 8) * 4);
                            int seed_backup_dst = smem_gate_addr + prep_stage * 41984 + (unsigned int)((2816 + seed_item * 8 + 1) * 4);
                            for (int seed_backup_word = 0; seed_backup_word < 4; seed_backup_word++) {
                                unsigned int seed_backup_pair[1];
                                asm volatile("ld.shared.b32 %0, [%1];" : "=r"(*reinterpret_cast<uint32_t*>(&seed_backup_pair[0])) : "r"(seed_backup_src + seed_backup_word * 8));
                                asm volatile("st.shared.b32 [%0], %1;" :: "r"(seed_backup_dst + seed_backup_word * 8), "r"((seed_backup_pair[0])));
                            }
                        }
                    }
                }
                ...
                if (pair_row_base >= pair_col_base) {
                    ...
                    int _shfl_xor_11 = __shfl_xor_sync(0xFFFFFFFF, lane, 1);
                    int late_lane = _shfl_xor_11 ^ 1;
                    int seed_lane_row = late_lane % 16;
                    int seed_lane_col = late_lane / 16 * 8;
                ...
                int _shfl_xor_12 = __shfl_xor_sync(0xFFFFFFFF, prep_tid, 1);
                int late_reload_tid = _shfl_xor_12 ^ 1;
                #pragma unroll 1
                for (int work_pass_1 = 0; work_pass_1 < 4; work_pass_1++) {
                    int work_item_1 = work_pass_1 * 128 + late_reload_tid;
                    int row_2 = work_item_1 / 16;
                    int segment_2 = work_item_1 % 16;
                    unsigned int sparse_packed[4];
                    int sparse_kr_base = smem_gate_addr + prep_stage * 41984 + (unsigned int)((row_2 * 128 + segment_2 * 8) * 4);
                    if (row_2 < 8) {
                        if ((segment_2 >> 1 & 1) == (row_2 & 1)) {
                            int collision_index = segment_2 / 4 * 2 + (segment_2 & 1);
                            int collision_flat = row_2 * 8 + collision_index;
                            sparse_kr_base = smem_gate_addr + prep_stage * 41984 + (unsigned int)((1024 + collision_flat * 8 + 1) * 4);
                        }
                    } else {
                        int config_1 = 0;
                        if (row_2 == 13) {
                        ...
                        int seed_config_1 = config_1;
                        int seed_mask_1 = seed_config_1 & 65535;
                        int seed_prefix_1 = seed_config_1 >> 16;
                        int seed_bit_1 = 1 << segment_2;
                        if ((seed_mask_1 & seed_bit_1) != 0) {
                            int _popc_1 = __popc(seed_mask_1 & seed_bit_1 - 1);
                            int seed_item_1 = seed_prefix_1 + _popc_1;
                            sparse_kr_base = smem_gate_addr + prep_stage * 41984 + (unsigned int)((2816 + seed_item_1 * 8 + 1) * 4);
                        }
                        if (row_2 == 31) {
                            sparse_kr_base = smem_gate_addr + prep_stage * 41984 + (unsigned int)(17032 + segment_2 * 16);
                        }
                    }
                    for (int sparse_word = 0; sparse_word < 4; sparse_word++) {
                        int sparse_word_stride = 8;
                        if (row_2 == 31) {
                            sparse_word_stride = 4;
                        }
                        asm volatile("ld.shared.b32 %0, [%1];" : "=r"(*reinterpret_cast<uint32_t*>(&sparse_packed[sparse_word])) : "r"(sparse_kr_base + sparse_word * sparse_word_stride));
                    }
                    for (int workspace_word = 0; workspace_word < 4; workspace_word++) {
                        asm volatile("st.shared.b32 [%0], %1;" :: "r"((smem_kr_trans_addr + prep_stage * 41984 + (unsigned int)(segment_2 * 8 / 64 * 4096 + row_2 * 128 + segment_2 * 8 % 64 * 2 ^ (segment_2 * 8 / 64 * 4096 + row_2 * 128 + segment_2 * 8 % 64 * 2 >> 7 & 7) << 4)) + (unsigned int)(workspace_word * 4)), "r"((sparse_packed[workspace_word])));
                    }
                }
                if (prep_instance == 0) {
                    asm volatile("barrier.sync 11, 128;" ::: "memory");
                ...
                if (prep_tid < 128) {
                    ...
                    float _exp2_3 = approx_exp2(total_log2);
                ...
                if (prep_local_warp >= 2) {
                    ...
                    for (int restore_pass = 0; restore_pass < 6; restore_pass++) {
                        ...
                        for (int word_4 = 0; word_4 < 4; word_4++) {
                            asm volatile("st.shared.b32 [%0], %1;" :: "r"((smem_kd_addr + prep_stage * 41984 + (unsigned int)(restore_segment * 8 / 64 * 4096 + restore_row * 128 + restore_segment * 8 % 64 * 2 ^ (restore_segment * 8 / 64 * 4096 + restore_row * 128 + restore_segment * 8 % 64 * 2 >> 7 & 7) << 4)) + (unsigned int)(word_4 * 4)), "r"((packed_2_1[word_4])));
                        }
                    }
                }
                if (prep_local_warp == 0) {
                ...
                if (prep_local_warp == 0) {
                    ...
                    for (int restore_pass_1 = 0; restore_pass_1 < 4; restore_pass_1++) {
                        ...
                        for (int word_7 = 0; word_7 < 4; word_7++) {
                            asm volatile("st.shared.b32 [%0], %1;" :: "r"((smem_kd_addr + prep_stage * 41984 + (unsigned int)(restore_segment_1 * 8 / 64 * 4096 + restore_row_1 * 128 + restore_segment_1 * 8 % 64 * 2 ^ (restore_segment_1 * 8 / 64 * 4096 + restore_row_1 * 128 + restore_segment_1 * 8 % 64 * 2 >> 7 & 7) << 4)) + (unsigned int)(word_7 * 4)), "r"((packed_2_2[word_7])));
                        }
                    }
                }
                asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
