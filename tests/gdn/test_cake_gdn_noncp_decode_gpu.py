...
def _fp32_mtp_is_performance_excluded(
    *, arch: cake_gdn.CakeGDNArch, batch_size: int, seq_len: int
) -> bool:
    return (batch_size, seq_len) == (1, 2) or (
        arch == "sm_100a" and (batch_size, seq_len) == (4, 4)
    )
...
def _make_fp32_t1_indexed_inputs(
    *, batch_size: int, scale: float
) -> dict[str, torch.Tensor]:
    torch.manual_seed(2050 + int(scale == 1.0))
    device = torch.device("cuda")
    num_q_heads, num_v_heads = 16, 32
    state = torch.randn(
        batch_size,
    ...
    return {
        "q": torch.randn(
            batch_size, 1, num_q_heads, _HEAD_SIZE, device=device, dtype=torch.bfloat16
        ),
        "k": torch.randn(
            batch_size, 1, num_q_heads, _HEAD_SIZE, device=device, dtype=torch.bfloat16
        ),
        "v": torch.randn(
            batch_size, 1, num_v_heads, _HEAD_SIZE, device=device, dtype=torch.bfloat16
        ),
        "state": state,
        ...
        "dt_bias": torch.randn(num_v_heads, device=device, dtype=torch.float32).mul_(0.1),
        "b": torch.randn(
            batch_size, 1, num_v_heads, device=device, dtype=torch.bfloat16
        ),
        "out": torch.full(
            (batch_size, 1, num_v_heads, _HEAD_SIZE),
            7.5,
            device=device,
            dtype=torch.bfloat16,
        ),
        "initial_state_indices": indices.clone(),
        "output_state_indices": indices.clone(),
    }


def _load_fp32_t1_indexed(*, batch_size: int, scale: float):
    route = cake_gdn.select_cake_gdn_decode_variant(
        arch=_arch(),
        batch_size=batch_size,
        io_dtype="bfloat16",
...
def _launch_fp32_t1_indexed(
    entry, tensors: dict[str, torch.Tensor], *, batch_size: int
) -> None:
    entry(
        tensors["q"],
        tensors["k"],
...
def test_exported_fp32_mtp_raw_abi_rejects_cache_and_grid_mismatch() -> None:
    ...
    # Keep this input contiguous so the test reaches the intended shape gate
    # instead of being rejected earlier by the independent ABI layout check.
    tensors["intermediate_state"] = cache[:, :1].contiguous()
...
def test_exported_fp32_mtp_performance_excluded_rows_fail_closed() -> None:
    arch = _arch()
    rows = [(1, 2)]
    if arch == "sm_100a":
        rows.append((4, 4))
    for batch_size, seq_len in rows:
        with pytest.raises(
            cake_gdn.CakeGDNUnsupportedError,
            match="not performance-promoted",
        ):
            _load_fp32_mtp(batch_size=batch_size, seq_len=seq_len)


@pytest.mark.parametrize(
    ("scale", "variant_name"),
    [
        (_SCALE, "decode_gdn_decode_pretranspose_splitv8_23d4a6f854ff"),
        (1.0, "decode_gdn_decode_pretranspose_splitv8_2c48580d5a19"),
    ],
)
def test_exported_fp32_t1_equal_pool_indices_do_not_access_pool(
    scale: float, variant_name: str
) -> None:
    batch_size = 128
    tensors = _make_fp32_t1_indexed_inputs(batch_size=batch_size, scale=scale)
    state = tensors["state"]
    initial_indices = tensors["initial_state_indices"]
    output_indices = tensors["output_state_indices"]
    initial_indices[0] = state.shape[0]
    output_indices[1] = state.shape[0]
    protected = state[:2].clone()
    route, entry = _load_fp32_t1_indexed(batch_size=batch_size, scale=scale)
    assert route.route_id.endswith("indexed_fp32_t1_splitv8")
    assert route.variant_name == variant_name

    _launch_fp32_t1_indexed(entry, tensors, batch_size=batch_size)
    torch.cuda.synchronize()

    assert torch.all(tensors["out"][0] == 0)
    assert torch.equal(state[:2], protected)


@pytest.mark.parametrize(
    ("batch_size", "seq_len", "variant_name"),
    [
        (4, 4, "decode_gdn_decode_pretranspose_mtp_t4_splitv8_337c41e6a804"),
        (
            16,
            4,
...
def test_exported_fp32_mtp_equal_pool_indices_preserve_outputs_and_state(
    batch_size: int, seq_len: int, variant_name: str
) -> None:
    arch = _arch()
    if _fp32_mtp_is_performance_excluded(
        arch=arch, batch_size=batch_size, seq_len=seq_len
    ):
        pytest.skip("row is covered by the exact performance-exclusion test")
    tensors = _make_fp32_mtp_inputs(batch_size=batch_size, seq_len=seq_len)
    state = tensors["state"]
    initial_indices = tensors["initial_state_indices"]
    initial_indices[0] = state.shape[0]
    ...
    if seq_len == 4:
        tensors["output_state_indices"][1] = state.shape[0]
    ...
    assert torch.all(tensors["out"][0] == 7.5)
    assert torch.all(tensors["intermediate_state"][0] == 3.25)
    if seq_len == 2:
        assert torch.equal(tensors["state_backing"], backing_before)
    else:
        read_indices = tensors["initial_state_indices"]
        write_indices = tensors["output_state_indices"]
        authorized = (
            (read_indices >= 0)
            & (read_indices < state.shape[0])
            & (write_indices >= 0)
            & (write_indices < state.shape[0])
        )
        touched_slots = write_indices[authorized].long()
        protected = torch.ones(state.shape[0], device=state.device, dtype=torch.bool)
        protected[touched_slots] = False
        assert torch.equal(state[protected], backing_before[:, ::2][protected])
        assert torch.equal(tensors["state_backing"][:, 1::2], backing_before[:, 1::2])


@pytest.mark.parametrize(("batch_size", "seq_len"), [(4, 4), (16, 4), (64, 4)])
...
@pytest.mark.parametrize(("batch_size", "seq_len"), [(4, 4), (16, 4)])
...
def test_exported_bf16_serving_rows_match_torch_on_caller_stream(
    ...
    if seq_len == 4 and num_q_heads == 4 and _arch() == "sm_103a":
        pytest.skip("SM103 TP4 T=4 rows are accuracy fail-closed")
...
def test_exported_bf16_invalid_output_indices_do_not_write() -> None:
    batch_size, seq_len, num_q_heads, num_v_heads = 8, 2, 16, 64
    tensors = _make_bf16_serving_inputs(
        batch_size=batch_size,
        seq_len=seq_len,
        num_q_heads=num_q_heads,
    ...
    expected_out, _, expected_final = _bf16_serving_reference(tensors)
    state = tensors["state"]
    backing = tensors["state_backing"]
    output_indices = tensors["output_state_indices"]
    assert isinstance(state, torch.Tensor)
    ...
    active_rows = torch.arange(2, batch_size, device=state.device)
    active_slots = output_indices.index_select(0, active_rows).long()
    torch.testing.assert_close(
        state.index_select(0, active_slots).float(),
        expected_final.index_select(0, active_rows).float(),
        atol=1e-2,
        rtol=1e-2,
    )
    touched = torch.zeros(state.shape[0], device=state.device, dtype=torch.bool)
    touched[active_slots] = True
    assert torch.equal(state[~touched], backing_before[:, :num_v_heads][~touched])
    assert torch.equal(backing[:, num_v_heads:], backing_before[:, num_v_heads:])
...
def test_exported_bf16_equal_pool_indices_are_bounded_for_every_variant(
    batch_size: int,
    seq_len: int,
    num_q_heads: int,
    num_v_heads: int,
    ...
    if variant_name.endswith("6b2e78ff65e0") and _arch() == "sm_103a":
        with pytest.raises(cake_gdn.CakeGDNUnsupportedError):
            cake_gdn.load_cake_gdn_kernel(variant_name, "sm_103a")
        with pytest.raises(cake_gdn.CakeGDNUnsupportedError):
            _load_bf16_serving(
                batch_size=batch_size,
                seq_len=seq_len,
    ...
    safe_indices = initial_indices.clone()
    safe_indices[0] = 0
    reference_tensors["initial_state_indices"] = safe_indices
    expected_out, expected_cache, expected_final = _bf16_serving_reference(
        reference_tensors
    )
    backing_before = backing.clone()
    route, entry = _load_bf16_serving(
    ...
    if isinstance(cache, torch.Tensor):
        ...
        valid_rows = torch.arange(2, batch_size, device=state.device)
        valid_slots = output_indices.index_select(0, valid_rows).long()
        torch.testing.assert_close(
            state.index_select(0, valid_slots).float(),
            expected_final.index_select(0, valid_rows).float(),
            atol=1e-2,
            rtol=1e-2,
        )
        touched = torch.zeros(state.shape[0], device=state.device, dtype=torch.bool)
        touched[valid_slots] = True
        assert torch.equal(state[~touched], backing_before[:, :num_v_heads][~touched])
        assert torch.equal(backing[:, num_v_heads:], backing_before[:, num_v_heads:])


def test_exported_bf16_invalid_read_indices_use_null_slot_without_writing() -> None:
    batch_size, seq_len, num_q_heads, num_v_heads = 8, 2, 16, 64
    tensors = _make_bf16_serving_inputs(
        batch_size=batch_size,
        seq_len=seq_len,
    ...
    assert isinstance(output_indices, torch.Tensor)
    initial_indices[0] = -1
    initial_indices[1] = state.shape[0]
    reference_tensors = dict(tensors)
    safe_indices = initial_indices.clone()
    safe_indices[:2] = 0
    reference_tensors["initial_state_indices"] = safe_indices
    expected_out, _, expected_final = _bf16_serving_reference(reference_tensors)
    backing_before = backing.clone()
    _, entry = _load_bf16_serving(
        batch_size=batch_size,
        seq_len=seq_len,
...
def test_exported_bf16_verify_is_cuda_graph_safe(
    ...
    if num_q_heads == 4 and _arch() == "sm_103a":
        pytest.skip("SM103 TP4 T=4 rows are accuracy fail-closed")
    tensors = _make_bf16_serving_inputs(
        batch_size=batch_size,
        seq_len=seq_len,
