...
HEAD_DIM = 128
_FP8 = "float8_e4m3fn"
_EXACT_GRID_ENV = "FLASHINFER_CAKE_MSA_EXPERIMENTAL_EXACT_GRID"


def _exact_grid_metadata() -> dict[str, Any]:
    return {
        "q": torch.empty((256, 64, HEAD_DIM), dtype=torch.bfloat16, device="meta"),
        "k": torch.empty(
            (2048, 4, BLOCK_SIZE, HEAD_DIM),
        ...
        "q2k_indices": torch.empty(
            (4, 256, 16), dtype=torch.int32, device="meta"
        ),
        "page_table": torch.empty((32, 64), dtype=torch.int32, device="meta"),
        "seqused_k": torch.empty((32,), dtype=torch.int32, device="meta"),
        "cu_seqlens_k": torch.empty((33,), dtype=torch.int32, device="meta"),
        "route": "m128",
        "q_tile": 16,
        "batch_size": 32,
        "seqlen_q": 8,
...
def test_experimental_exact_grid_environment_is_strict(monkeypatch) -> None:
    from flashinfer.msa_ops._cake_sm100 import _experimental_exact_grid_enabled

    monkeypatch.delenv(_EXACT_GRID_ENV, raising=False)
    assert not _experimental_exact_grid_enabled()
    monkeypatch.setenv(_EXACT_GRID_ENV, "1")
    assert _experimental_exact_grid_enabled()
    monkeypatch.setenv(_EXACT_GRID_ENV, "yes")
    with pytest.raises(ValueError, match="must be 0 or 1"):
        _experimental_exact_grid_enabled()


def test_frozen_b32_q8_decode_exact_grid_is_env_gated() -> None:
    from flashinfer.msa_ops._cake_sm100 import _decode_prefill_grid

    metadata = _exact_grid_metadata()
    assert _decode_prefill_grid(
        **metadata, experimental_exact_grid=False
    ) == (47, 4, 1)
    assert _decode_prefill_grid(
        **metadata, experimental_exact_grid=True
    ) == (32, 4, 1)
...
@pytest.mark.parametrize(
    ("name", "value"),
    [
        pytest.param("route", "m64", id="different-route"),
        pytest.param("q_tile", 32, id="different-q-tile"),
        pytest.param("seqlen_q", 7, id="different-q-length"),
        pytest.param("paged", False, id="flat-layout"),
        pytest.param("causal", False, id="noncausal"),
        pytest.param("force_fused", None, id="automatic-fusion"),
        pytest.param("cu_seqlens_k", None, id="missing-frozen-cu-k"),
    ],
)
def test_experimental_exact_grid_rejects_nonfrozen_scalar_metadata(
    name: str, value: Any
) -> None:
    from flashinfer.msa_ops._cake_sm100 import _decode_prefill_grid
    ...
    metadata[name] = value
    expected_grid_x = (
        256 + int(metadata["q_tile"]) - 1
    ) // int(metadata["q_tile"]) + 31
    assert _decode_prefill_grid(
        **metadata, experimental_exact_grid=True
    ) == (expected_grid_x, 4, 1)


def test_experimental_exact_grid_rejects_nonfrozen_tensor_metadata() -> None:
    from flashinfer.msa_ops._cake_sm100 import _decode_prefill_grid

    metadata = _exact_grid_metadata()
    metadata["q2k_indices"] = torch.empty(
        (4, 256, 8), dtype=torch.int32, device="meta"
    )
    assert _decode_prefill_grid(
        **metadata, experimental_exact_grid=True
    ) == (47, 4, 1)
...
def test_decode_route_key_tracks_experimental_grid_mode() -> None:
    from flashinfer.msa_ops._cake_sm100 import _route_key

    q = torch.empty((1, 1, HEAD_DIM), dtype=torch.bfloat16)
    k = torch.empty((1, 1, HEAD_DIM), dtype=torch.bfloat16)
    v = torch.empty_like(k)
    q2k = torch.empty((1, 1, 4), dtype=torch.int32)
    kwargs = {
        "q": q,
        "k": k,
        "v": v,
    ...
    default_key = _route_key(**kwargs, experimental_exact_grid=False)
    exact_key = _route_key(**kwargs, experimental_exact_grid=True)

    assert default_key[:-1] == exact_key[:-1]
    assert default_key[-1] is False
    assert exact_key[-1] is True
