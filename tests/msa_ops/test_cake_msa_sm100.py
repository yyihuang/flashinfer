...
def test_mixed_fp8_q1_selects_m16_candidate() -> None:
    ...
    k = torch.empty((1, 4, HEAD_DIM), dtype=torch.float8_e4m3fn)
    route = _select_decode_route(
        q=q,
        k=k,
        cu_k=torch.empty(1, dtype=torch.int32),
        kv_lens=torch.empty(1, dtype=torch.int32),
        group_size=16,
        seqlen_q=1,
        paged=True,
        force_fused=True,
        workspace=None,
        route_key=("mixed-fp8-m16",),
        capturing=False,
    )

    assert route == ("m16", False, True)
    assert (
        _decode_variant(
            q_dtype=q.dtype,
            k_dtype=k.dtype,
            paged=True,
            specialized_m16=True,
        )
