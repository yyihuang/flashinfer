...
def test_boundary_tiny_generic_decode_counterfactual_is_exact_and_opt_in(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    from flashinfer.msa_ops._cake_sm100 import _select_decode_route

    env_name = "FLASHINFER_CAKE_MSA_EXPERIMENTAL_TINY_DECODE_ROUTE"
    q = torch.empty((2, 8, HEAD_DIM), dtype=torch.bfloat16)
    k = torch.empty((6, 1, BLOCK_SIZE, HEAD_DIM), dtype=torch.bfloat16)
    common = {
        "q": q,
        "k": k,
        ...
        "force_fused": True,
        "workspace": None,
        "route_key": ("boundary-kv257-counterfactual",),
        "capturing": False,
    }
    ...
    monkeypatch.setenv(env_name, "generic")
    ...
    assert _select_decode_route(**neighboring) == ("m16", False, True)

    monkeypatch.setenv(env_name, "typo")
    with pytest.raises(ValueError, match="must be unset or 'generic'"):
        _select_decode_route(**common)
