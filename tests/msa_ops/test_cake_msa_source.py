...
def test_cake_msa_fp8_decode_sources_support_persistent_split_reduction():
    """Candidate B relies on this frozen mechanism for flat and paged FP8."""
    ...
    required_tokens = (
        ...
        "msa_split_policy == 1",
        "partial_O",
        "atomicAdd(&split_completion[",
    )
    for variant in ("decode_fp8_flat", "decode_fp8_paged"):
        device_source = (
            _CAKE_MSA_CSRC_DIR / _device_name(variant)
        ).read_text(encoding="utf-8")
        for token in required_tokens:
            assert token in device_source, f"{variant} is missing {token!r}"
