...
_SOURCE_SHA256 = {
    ...
    "cake_msa_decode_m16_bf16_paged_binding.cu": "ffdc14a13347cca284fcd14a612dfd284999a346b1bdbac8feda39c4c43949d1",
...
def test_cake_msa_boundary_prototype_uses_two_pass_cta_softmax():
    binding_source = (
        _CAKE_MSA_CSRC_DIR / _binding_name("decode_m16_bf16_paged")
    ).read_text(encoding="utf-8")

    assert "flashinfer_msa_tiny_two_pass_bf16_paged" in binding_source
    assert "constexpr int kScoreCount = kTopK * kBlockSize;" in binding_source
    assert "__shared__ float scores[kScoreCount];" in binding_source
    assert "for (int token = warp; token < slot_valid; token += kWarps)" in binding_source
    assert "scores[slot * kBlockSize + token] = score;" in binding_source
    assert "float local_max = fmaxf(scores[tid], scores[tid + 256]);" in binding_source
    assert "float local_sum = weight0 + weight1;" in binding_source
    assert "if (tid < 128)" in binding_source
    assert "arg_is_causal == 1" in binding_source
    assert (
        "dim3(static_cast<uint32_t>(tiny_total_q * tiny_num_q_heads), 1u, 1u)"
        in binding_source
    )
    assert "dim3(256u, 1u, 1u), tiny_kargs" in binding_source
