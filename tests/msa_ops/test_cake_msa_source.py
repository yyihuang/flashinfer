...
_SOURCE_SHA256 = {
    ...
    "cake_msa_decode_m16_bf16_paged_binding.cu": "96472a39143c951ebdfd46ba547ad81aeb3217d49e24709a11a85817193824ab",
...
def test_cake_msa_boundary_prototype_uses_one_warp_per_query_head():
    binding_source = (
        _CAKE_MSA_CSRC_DIR / _binding_name("decode_m16_bf16_paged")
    ).read_text(encoding="utf-8")

    assert "__global__ __launch_bounds__(32) void " in binding_source
    assert "flashinfer_msa_tiny_head_cta_bf16_paged" in binding_source
    assert "const int request = work / num_q_heads;" in binding_source
    assert "const int q_head = work - request * num_q_heads;" in binding_source
    assert "arg_is_causal == 1" in binding_source
    assert (
        "dim3(static_cast<uint32_t>(tiny_total_q * tiny_num_q_heads), 1u, 1u)"
        in binding_source
    )
    assert "dim3(32u, 1u, 1u), tiny_kargs" in binding_source
