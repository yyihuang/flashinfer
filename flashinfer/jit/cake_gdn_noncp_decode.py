...
_EXPORT_SCHEMA = "flashinfer-gdn-noncp-decode-standalone-export-v3"
_MANIFEST_SHA256 = "ef3ab40fe5c4fb6308704b2fb83afb371efa9c3ef502028cf3bc12d7101d394a"
_PUBLIC_BASELINE_SOURCES = {
...
_FP16_INDEXED_STATE_SEQ_LENS = {
    (128,),
    (256,),
    (128, 192, 64),
    (64, 512),
}
_FP16_INDEXED_STATE_HEADS = {16, 32}
_FP16_INDEXED_STATE_POOL_PADDING = {0, 96}
...
def _manifest() -> dict[str, Any]:
    ...
    forbidden_provenance = {"generator_commit", "baseline_revisions", "source_pins"}
    leaked_provenance = forbidden_provenance.intersection(manifest)
    if leaked_provenance:
        raise RuntimeError(
            "Cake GDN manifest contains private provenance fields: "
            f"{sorted(leaked_provenance)!r}"
        )
    observed = (
        manifest.get("schema"),
        manifest.get("architectures"),
        manifest.get("public_baseline_sources"),
        manifest.get("source_only"),
        manifest.get("binary_artifacts"),
        manifest.get("manifest_only"),
...
def _cuda_record(record: dict[str, Any], arch: CakeGDNArch) -> dict[str, Any]:
    ...
    if len(outputs) != 1:
        raise CakeGDNUnsupportedError(
            f"kernel {record['name']!r} does not support {arch}"
        )
    return outputs[0]
...
@functools.cache
def _nvcc_identity(nvcc: Path) -> str:
    result = subprocess.run(
        [str(nvcc), "--version"],
        stdout=subprocess.PIPE,
        stderr=subprocess.STDOUT,
    ...
    if result.returncode != 0:
        raise RuntimeError(f"failed to query nvcc version at {nvcc}:\n{result.stdout}")
    return f"{nvcc.resolve()}\n{result.stdout}"
...
def _prefill_schedule_attr(
    *,
    ...
    if io_dtype == "float16" and state_dtype == "float16":
        return f"{stem}_initial_f16io_f16state"
...
def _exclusion_reason(
    *, domain: str, arch: CakeGDNArch, selector: dict[str, Any]
) -> str | None:
    """Return the sealed fail-closed reason for one exact host selector."""

    exclusions = _manifest().get("exclusions")
    if not isinstance(exclusions, dict) or exclusions.get("policy") != "fail_closed":
        raise RuntimeError("Cake GDN manifest lost its fail-closed exclusion policy")
    records = exclusions.get("records")
    if not isinstance(records, list):
        raise RuntimeError("Cake GDN manifest exclusion records are malformed")
    matches = [
        record.get("reason")
        ...
        and record.get("selector") == selector
    ]
    if len(matches) > 1 or any(not isinstance(reason, str) for reason in matches):
        raise RuntimeError("Cake GDN manifest contains ambiguous exclusion records")
    return matches[0] if matches else None


def _raise_if_excluded(
    *, domain: str, arch: CakeGDNArch, selector: dict[str, Any]
) -> None:
    reason = _exclusion_reason(domain=domain, arch=arch, selector=selector)
    if reason is not None:
        raise CakeGDNUnsupportedError(f"Cake GDN selector is fail closed: {reason}")


def _prefill_selector(
    *,
    io_dtype: str,
    state_dtype: str,
    seq_lens: tuple[int, ...],
...
def _resolved_prefill_seq_lens(
    *,
    arch: CakeGDNArch,
    io_dtype: str,
    state_dtype: str,
    ...
    """Resolve ordered lengths without copying device planning metadata."""

    if seq_lens is not None:
        resolved = tuple(int(length) for length in seq_lens)
        if (
            len(resolved) != num_seqs
            or any(length <= 0 for length in resolved)
            or sum(resolved) != total_seq_len
            or max(resolved) != max_seq_len
        ):
            raise CakeGDNUnsupportedError(
                "ordered seq_lens do not match num_seqs/total_seq_len/max_seq_len"
            )
        return resolved
    if num_seqs == 1:
        return (total_seq_len,)
    if total_seq_len == num_seqs * max_seq_len:
        return (max_seq_len,) * num_seqs

    partial = _prefill_selector(
        io_dtype=io_dtype,
        state_dtype=state_dtype,
        seq_lens=(),
        num_q_heads=num_q_heads,
    ...
    records = _manifest()["exclusions"]["records"]
    for record in records:
        selector = record["selector"]
        ordered = selector.get("seq_lens")
        if (
            record["domain"] == "prefill"
            and record["architecture"] == arch
            and isinstance(ordered, list)
            and len(ordered) == num_seqs
            and sum(ordered) == total_seq_len
            and max(ordered) == max_seq_len
            and all(
            ...
            raise CakeGDNUnsupportedError(
                "ordered seq_lens are required to disambiguate a fail-closed "
                "Cake GDN prefill selector"
            )
    return None
...
def select_cake_gdn_prefill_variant(
    *,
    ...
    preallocated_out: bool = False,
    state_pool_padding: int = 0,
) -> CakeGDNRoute:
    ...
    resolved_seq_lens = _resolved_prefill_seq_lens(
        arch=arch,
        io_dtype=io_dtype,
        state_dtype=state_dtype,
        num_seqs=num_seqs,
    ...
    exact_fp16_indexed_state = (
        io_dtype == "float16"
        and state_dtype == "float16"
        and use_initial_state
        and store_final_state
        and not enable_checkpoints
        and use_state_indices
        and preallocated_out
        and num_q_heads == num_k_heads == num_v_heads
        and num_q_heads in _FP16_INDEXED_STATE_HEADS
        and resolved_seq_lens in _FP16_INDEXED_STATE_SEQ_LENS
        and state_pool_padding in _FP16_INDEXED_STATE_POOL_PADDING
    )
    if low_precision_state and (
        not exact_fp16_indexed_state
        and (
            io_dtype != "bfloat16"
        ...
        raise CakeGDNUnsupportedError(
            "low-precision state requires BF16 I/O with a packed state pool or "
            "one frozen FP16 indexed-state row; both require initial+final state "
            "and no checkpoints"
        )
    ...
    if enable_checkpoints and not (
        ...
        raise CakeGDNUnsupportedError(
            ...
            "an exact SGLang TP4 BF16 indexed B1/T103 or B7/T421 contract"
        )
    if (
        use_state_indices
        and not enable_checkpoints
        and not sglang_tp4_noncheckpoint_route
        and not exact_fp16_indexed_state
    ):
        raise CakeGDNUnsupportedError(
    ...
    if low_precision_state and not dvsplit and not exact_fp16_indexed_state:
        raise CakeGDNUnsupportedError(
    ...
    if resolved_seq_lens is not None:
        _raise_if_excluded(
            domain="prefill",
            arch=arch,
            selector=_prefill_selector(
                io_dtype=io_dtype,
                state_dtype=state_dtype,
    ...
    *,
    ...
    if state_dtype == "bfloat16":
        ...
        if (
            ...
            raise CakeGDNUnsupportedError(
                "BF16 decode is limited to the fifteen exact promoted indexed/verify rows"
            )
