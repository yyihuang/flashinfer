...
_TINY_SINGLE_CTA_ENV = "FLASHINFER_CAKE_MSA_TINY_SINGLE_CTA_CANDIDATE"
...
def _tiny_single_cta_enabled() -> bool:
    """Return whether the frozen KV257 one-CTA experiment is enabled."""

    value = os.environ.get(_TINY_SINGLE_CTA_ENV, "0")
    if value not in {"0", "1"}:
        raise ValueError(f"{_TINY_SINGLE_CTA_ENV} must be 0 or 1, got {value!r}")
    return value == "1"


def _is_frozen_tiny_single_cta_candidate(
    *,
    q: torch.Tensor,
    k: torch.Tensor,
    group_size: int,
    ...
    """Match only B2/Q1/KV257/Hq8/Hkv1/TopK4 paged BF16."""
    ...
    return (
        ...
        and tuple(q.shape) == (2, 8, _HEAD_DIM)
        and tuple(k.shape) == (6, 1, _BLOCK_SIZE, _HEAD_DIM)
    )
...
def _decode_persistent_launch_plan(
    *,
    ...
    single_cta_persistent: bool,
) -> tuple[int, int, tuple[int, int, int]]:
    """Resolve the persistent task count and grid for candidate E."""

    if single_cta_persistent and not persistent_unsplit:
        raise ValueError("single-CTA decode requires persistent_unsplit")
    max_splits = 1 if persistent_unsplit else topk // 2
    max_task_claims = int(total_q) * int(num_kv_heads) * max_splits
    grid_x = 1 if single_cta_persistent else min(num_sms, max_task_claims)
    return max_splits, max_task_claims, (grid_x, 1, 1)
...
def _run_decode_module(
    *,
    ...
    force_fused: Optional[bool],
    persistent_unsplit: bool,
    single_cta_persistent: bool = False,
    derive_q_offset: bool,
    workspace: Optional[MSASparseAttentionWorkspace],
    capturing: bool,
    stream_ptr: int,
    ...
    if force_fused is True:
        ...
        max_splits, max_task_claims, grid = _decode_persistent_launch_plan(
            ...
            single_cta_persistent=single_cta_persistent,
        )
        qo_indptr = i32_dummy
...
def _select_decode_route(
    *,
    ...
    if tiny_single_cta and _is_frozen_tiny_single_cta_candidate(
        q=q,
        k=k,
        ...
        # Counterfactual E: one persistent M16 CTA claims both request tasks.
...
def cake_msa_sparse_decode_attention(
    ...
    *,
    ...
    with context:
        ...
        tiny_single_cta = _tiny_single_cta_enabled() and (
        ...
        route_key = _route_key(
            ...
            seqlen_q=seqlen_q,
            force_fused=force_fused,
            tiny_single_cta=tiny_single_cta,
        )
        ...
        if route in {"m64", "m128"}:
            ...
            _run_decode_module(
                ...
                persistent_unsplit=persistent_unsplit,
                single_cta_persistent=tiny_single_cta,
                derive_q_offset=derive_q_offset,
                workspace=workspace,
                capturing=capturing,
                stream_ptr=stream_ptr,
