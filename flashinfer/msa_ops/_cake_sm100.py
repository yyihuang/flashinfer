...
_EXPERIMENTAL_TINY_DECODE_ROUTE_ENV = (
    "FLASHINFER_CAKE_MSA_EXPERIMENTAL_TINY_DECODE_ROUTE"
)
_EXPERIMENTAL_GENERIC_DECODE = "generic"
...
def _use_experimental_generic_tiny_decode(
    *,
    q: torch.Tensor,
    k: torch.Tensor,
    group_size: int,
    ...
    """Return whether the frozen KV257 boundary experiment is enabled."""

    setting = os.environ.get(_EXPERIMENTAL_TINY_DECODE_ROUTE_ENV, "")
    if setting not in ("", _EXPERIMENTAL_GENERIC_DECODE):
        raise ValueError(
            f"{_EXPERIMENTAL_TINY_DECODE_ROUTE_ENV} must be unset or "
            f"{_EXPERIMENTAL_GENERIC_DECODE!r}, got {setting!r}"
        )
    if setting != _EXPERIMENTAL_GENERIC_DECODE:
        return False

    # Keep this counterfactual exact: opting in must not reroute neighboring
    # production or coverage calls. The frozen boundary row materializes six
    # page-128 K/V pages for B2 x KV257, with Hq8/Hkv1 and forced fused decode.
    return (
        paged
        and seqlen_q == 1
...
def _select_decode_route(
    *,
    ...
    if use_m16 and _use_experimental_generic_tiny_decode(
        q=q,
        k=k,
