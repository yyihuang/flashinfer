...
# Candidate-B experiment knob.  The default is deliberately off so the public
# ``force_fused`` contract is unchanged outside an explicitly recorded
# benchmark run.  With the knob enabled, Q1 BF16-query/FP8-KV decode uses the
# already-generated persistent split/reduction schedule even when the frozen
# benchmark row requests ``force_fused=True``.
_FP8_Q1_SPLIT_CANDIDATE_ENV = "FLASHINFER_CAKE_MSA_FP8_Q1_SPLIT_CANDIDATE"
...
def _fp8_q1_split_candidate_enabled() -> bool:
    """Return whether the explicitly opt-in Candidate-B route is enabled."""

    value = os.environ.get(_FP8_Q1_SPLIT_CANDIDATE_ENV, "0")
    if value not in {"0", "1"}:
        raise ValueError(f"{_FP8_Q1_SPLIT_CANDIDATE_ENV} must be 0 or 1, got {value!r}")
    return value == "1"


def _decode_persistent_launch_plan(
    *,
    total_q: int,
    num_kv_heads: int,
    topk: int,
    ...
    """Resolve the persistent decode split/task/grid dimensions.

    The generated Q1 decode schedule consumes two selected KV blocks per split,
    hence the TopK-dependent split capacity is ``topk // 2``.  Keeping this
    calculation in the host plan makes the candidate's task decomposition
    explicit and independently testable.
    """

    max_splits = 1 if persistent_unsplit else max(1, topk // 2)
    max_task_claims = int(total_q) * int(num_kv_heads) * max_splits
    return max_splits, max_task_claims, (min(num_sms, max_task_claims), 1, 1)
...
def _route_key(
    *,
    ...
    seqlen_q: int,
    force_fused: Optional[bool],
    experimental_fp8_q1_split: bool = False,
) -> tuple:
...
def _select_decode_route(
    *,
    ...
    if (
        ...
        # This opt-in experiment intentionally bypasses the direct fused launch
        # requested by the frozen performance rows.  The default-off gate keeps
        # normal callers' explicit ``force_fused`` semantics intact while the
        # split schedule's performance ceiling is measured.
...
def cake_msa_sparse_decode_attention(
    ...
    *,
    ...
    with context:
        ...
        experimental_fp8_q1_split = _fp8_q1_split_candidate_enabled()
        route_key = _route_key(
            ...
            experimental_fp8_q1_split=experimental_fp8_q1_split,
        )
