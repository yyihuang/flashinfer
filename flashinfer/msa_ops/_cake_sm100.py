...
if TYPE_CHECKING:
    from ..jit.cake_msa import CakeMSATarget, CakeMSAVariant
...
_EXPERIMENTAL_EXACT_SPARSE_ROUTE_ENV = (
    "FLASHINFER_CAKE_MSA_EXPERIMENTAL_EXACT_SPARSE_ROUTE"
)
...
def _select_target(device: torch.device) -> "CakeMSATarget":
...
def _get_module(variant: "CakeMSAVariant", target: "CakeMSATarget"):
    from ..jit.cake_msa import get_cake_msa_module

    return get_cake_msa_module(variant, target)


def _experimental_exact_sparse_route_enabled() -> bool:
    """Return whether the frozen exact-selected schedule experiment is enabled."""

    value = os.environ.get(_EXPERIMENTAL_EXACT_SPARSE_ROUTE_ENV, "0")
    if value not in {"0", "1"}:
        raise ValueError(
            f"{_EXPERIMENTAL_EXACT_SPARSE_ROUTE_ENV} must be 0 or 1, got {value!r}"
        )
    return value == "1"
...
def _run_prefill_module(
    *,
    variant: "CakeMSAVariant",
    target: "CakeMSATarget",
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    out: torch.Tensor,
...
def _route_key(
    *,
    ...
    seqlen_q: int,
    force_fused: Optional[bool],
    experimental_exact_sparse: bool = False,
) -> tuple:
    ...
    return (
        ...
        *optional_tensors,
        seqlen_q,
        force_fused,
        experimental_exact_sparse,
    )


def _is_frozen_q8_decode_exact_candidate(
    *,
    q: torch.Tensor,
    k: torch.Tensor,
    group_size: int,
    ...
    """Match the two frozen paged Q8 decode counterfactuals."""
    ...
    official_small = tuple(q.shape) == (256, 64, _HEAD_DIM) and tuple(k.shape) == (
        2048,
        4,
        _BLOCK_SIZE,
        _HEAD_DIM,
    )
    official_long = tuple(q.shape) == (512, 64, _HEAD_DIM) and tuple(k.shape) == (
        32768,
        4,
        _BLOCK_SIZE,
        _HEAD_DIM,
    )
    return common and (official_small or official_long)


def _is_frozen_prefill_exact_candidate(
    *,
    q: torch.Tensor,
    k: torch.Tensor,
    q2k_indices: torch.Tensor,
    ...
    """Match the two frozen prefill rows used by counterfactual D2."""

    if batch_size != 3 or return_temperature_lse:
        return False
    mixed_fp8_flat = (
        not paged
        and group_size == 16
        ...
        and tuple(k.shape) == (24576, 2, _HEAD_DIM)
    ...
    bf16_paged = (
        ...
        and tuple(k.shape) == (192, 2, _BLOCK_SIZE, _HEAD_DIM)
        and tuple(q2k_indices.shape) == (2, 12288, 4)
    )
    return mixed_fp8_flat or bf16_paged
...
def _select_decode_route(
    *,
    ...
    if experimental_exact_sparse and _is_frozen_q8_decode_exact_candidate(
        q=q,
        k=k,
        ...
        # Counterfactual D1: visit the 32 selected blocks directly with the
        # M16 swap-AB body instead of scanning the 512-block visible range in
        # the folded-M128 prefill body.
...
def cake_msa_sparse_attention(
    ...
    with context:
        ...
        if (
            ...
            and _is_frozen_prefill_exact_candidate(
                q=q,
                k=k,
            ...
            # Counterfactual D2: reuse the exact-selected M16 body as a
            # multi-token physical schedule. ``record_tasks=seqlen_q`` lets
            # the generated direct path recover batch/local-query coordinates
            # while avoiding the visible-range union scan.
            variant = _decode_variant(
                q_dtype=q.dtype,
                k_dtype=k.dtype,
                paged=paged,
                specialized_m16=False,
            )
            _run_decode_module(
                variant=variant,
                target=target,
...
def cake_msa_sparse_decode_attention(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    q2k_indices: torch.Tensor,
    *,
    ...
    with context:
        ...
        experimental_exact_sparse = _experimental_exact_sparse_route_enabled()
        route_key = _route_key(
            ...
            seqlen_q=seqlen_q,
            force_fused=force_fused,
            experimental_exact_sparse=experimental_exact_sparse,
        )
        ...
        if route in {"m64", "m128"}:
            ...
            if route == "m64":
                variant: CakeMSAVariant = "prefill_m64_bf16_flat"
                q_tile = _M64_Q_TILE
            else:
                variant = _prefill_variant(
                    q_dtype=q.dtype,
...
__all__ = [
    ...
    "cake_msa_sparse_attention",
    "cake_msa_sparse_decode_attention",
    "cake_msa_topk_select",
    "is_cake_msa_device",
]
