...
_EXPERIMENTAL_EXACT_GRID_ENV = "FLASHINFER_CAKE_MSA_EXPERIMENTAL_EXACT_GRID"
...
def _experimental_exact_grid_enabled() -> bool:
    """Return whether the frozen-shape launch-grid experiment is enabled."""

    value = os.environ.get(_EXPERIMENTAL_EXACT_GRID_ENV, "0")
    if value not in {"0", "1"}:
        raise ValueError(
            f"{_EXPERIMENTAL_EXACT_GRID_ENV} must be 0 or 1, got {value!r}"
        )
    return value == "1"


def _matches_tensor_metadata(
...
def _decode_prefill_grid(
    *,
    ...
    experimental_exact_grid: bool,
) -> tuple[int, int, int]:
    """Resolve the M64/M128 decode launch grid without device synchronization."""

    grid_x = (int(q.shape[0]) + q_tile - 1) // q_tile + batch_size - 1
    frozen_b32_q8_kv8192 = (
        experimental_exact_grid
        and route == "m128"
        and q_tile == _M128_GQA16_Q_TILE
        and batch_size == 32
        and seqlen_q == 8
        and num_kv_heads == 4
        and paged
        ...
        and _matches_tensor_metadata(
            q,
            shape=(256, 64, _HEAD_DIM),
            dtype=torch.bfloat16,
        )
        and _matches_tensor_metadata(
            k,
            shape=(2048, 4, _BLOCK_SIZE, _HEAD_DIM),
            dtype=torch.bfloat16,
        )
        and _matches_tensor_metadata(
            v,
    ...
    if frozen_b32_q8_kv8192:
        # The frozen shape has one Q tile per sequence.  The conservative
        # ragged formula launches 47 CTAs because it cannot generally recover
        # per-sequence tile counts from total_q alone; here all 32 sequences
        # are statically known to contribute exactly one effective CTA.
        grid_x = batch_size
    return grid_x, num_kv_heads, 1
...
def _route_key(
    *,
    ...
    seqlen_q: int,
    force_fused: Optional[bool],
    experimental_exact_grid: bool = False,
) -> tuple:
...
def cake_msa_sparse_decode_attention(
    ...
    *,
    ...
    with context:
        ...
        experimental_exact_grid = _experimental_exact_grid_enabled()
        route_key = _route_key(
            ...
            experimental_exact_grid=experimental_exact_grid,
        )
